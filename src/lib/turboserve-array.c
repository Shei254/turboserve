...
#if !defined(turboserve_HAVE_BUILTIN_ADD_OVERFLOW)
...
{
    if (!(a->elements % turboserve_ARRAY_INCREMENT)) {
        void *new_base;
        size_t new_cap;

        if (UNLIKELY(
                add_overflow(a->elements, turboserve_ARRAY_INCREMENT, &new_cap))) {
            errno = EOVERFLOW;
            return NULL;
        }
                               ...
                               void *inline_storage)
{
    if (!a->base)
        ...
        return turboserve_array_append_heap(a, element_size);

    assert(a->elements <= turboserve_ARRAY_INCREMENT);

    if (a->elements == turboserve_ARRAY_INCREMENT) {
        void *new_base =
            reallocarray(NULL, 2 * turboserve_ARRAY_INCREMENT, element_size);

        if (UNLIKELY(!new_base))
            return NULL;

        a->base = memcpy(new_base, inline_storage,
                         turboserve_ARRAY_INCREMENT * element_size);
    }
...
static void coro_turboserve_array_free_heap(void *data)
{
    struct turboserve_array *array = data;

    turboserve_array_reset(array, NULL);
    free(array);
}

static void coro_turboserve_array_free_inline(void *data)
{
    struct turboserve_array *array = data;

    turboserve_array_reset(array, array + 1);
    free(array);
}
...
struct turboserve_array *coro_turboserve_array_new(struct coro *coro,
                                       ...
                                       bool inline_first)
{
    struct turboserve_array *array;

    assert(struct_size >= sizeof(struct turboserve_array));

    array = coro_malloc_full(coro, struct_size,
                             inline_first ? coro_turboserve_array_free_inline
                                          : coro_turboserve_array_free_heap);
    if (LIKELY(array))
        *array = (struct turboserve_array){.base = NULL, .elements = 0};
