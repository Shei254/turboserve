...
unsigned int parse_time_period(const char *str, unsigned int default_value)
{
    ...
    while (*str) {
        ...
        default:
            turboserve_status_warning("Ignoring unknown multiplier: %c",
                        multiplier);
        }
...
turboserve_SELF_TEST(parse_bool)
{
...
static void *parse_key_value(struct parser *parser)
{
    ...
    while ((lexeme = lexeme_ring_buffer_get_ptr_or_null(&parser->buffer))) {
        ...
        turboserve_strbuf_append_value(&parser->strbuf, &lexeme->value);

        if (!lexeme_ring_buffer_empty(&parser->buffer))
            turboserve_strbuf_append_char(&parser->strbuf, '_');
    }
    key_size = turboserve_strbuf_get_length(&parser->strbuf);
    turboserve_strbuf_append_char(&parser->strbuf, '\0');

    while ((lexeme = lex_next(&parser->lexer))) {
        if (UNLIKELY(turboserve_strbuf_get_length(&parser->strbuf) > 1ull<<20)) {
        ...
        case LEXEME_VARIABLE: {
            ...
            turboserve_strbuf_append_strz(&parser->strbuf, value);
        ...
        case LEXEME_EQUAL:
            turboserve_strbuf_append_char(&parser->strbuf, '=');
            break;
        ...
        case LEXEME_LINEFEED:
            line.key = turboserve_strbuf_get_buffer(&parser->strbuf);
...
static void *parse_section(struct parser *parser)
{
    ...
    struct config_line line = {
        ...
        .key = turboserve_strbuf_get_buffer(&parser->strbuf),
        .value = turboserve_strbuf_get_buffer(&parser->strbuf) + name_len + 1,
    };
...
static const struct config_line *parser_next_internal(struct parser *parser)
{
    while (parser->state) {
        ...
        turboserve_strbuf_reset(&parser->strbuf);
...
static bool parse_constants(struct config *config, const struct config_line *l)
{
    while ((l = config_read_line(config))) {
        ...
        case CONFIG_LINE_TYPE_LINE: {
            ...
            if (!k || !v)
                turboserve_status_critical("Can't allocate memory for constant");
...
config_open_path(const char *path, void **data, size_t *size)
{
    ...
    if (fd < 0) {
        turboserve_status_perror("Could not open configuration file: %s", path);
        return NULL;
    }
...
config_init_data(struct config *config, const void *data, size_t len)
{
    ...
    turboserve_strbuf_init(&config->parser.strbuf);
...
void config_close(struct config *config)
{
    ...
    turboserve_strbuf_free(&config->parser.strbuf);
    free(config);
}
                                      ...
                                      const struct config_line *current_line)
{
    ...
    turboserve_strbuf_init(&isolated->parser.strbuf);
    ...
    if (!find_section_end(isolated)) {
        ...
        turboserve_strbuf_free(&isolated->parser.strbuf);
        free(isolated);
