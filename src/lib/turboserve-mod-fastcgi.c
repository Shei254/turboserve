 ...
 * turboserve to proxy PHP-FPM through FastCGI.
 */
...
#include "turboserve-arena.h"
#include "turboserve-cache.h"
#include "turboserve-io-wrappers.h"
#include "turboserve-mod-fastcgi.h"
...
struct private_data {
    ...
    struct turboserve_value default_index;
...
static void add_param_len(struct turboserve_strbuf *strbuf,
                          const char *key,
                          size_t len_key,
                          const char *value,
                          size_t len_value)
{
    ...
    if (len_key <= 127) {
        turboserve_strbuf_append_char(strbuf, (char)len_key);
    } else {
        uint32_t len_net = htonl((uint32_t)len_key) | 1u << 31;
        turboserve_strbuf_append_str(strbuf, (char *)&len_net, 4);
    }

    if (len_value <= 127) {
        turboserve_strbuf_append_char(strbuf, (char)len_value);
    } else {
        uint32_t len_net = htonl((uint32_t)len_value) | 1u << 31;
        turboserve_strbuf_append_str(strbuf, (char *)&len_net, 4);
    }

    turboserve_strbuf_append_str(strbuf, key, len_key);
    turboserve_strbuf_append_str(strbuf, value, len_value);
}

static inline void
add_param(struct turboserve_strbuf *strbuf, const char *key, const char *value)
{
    return add_param_len(strbuf, key, strlen(key), value, strlen(value));
}

static inline void
add_int_param(struct turboserve_strbuf *strbuf, const char *key, ssize_t value)
{
    size_t len;
...
create_script_name(const void *keyptr, void *context, void *create_contex)
{
    ...
    const struct turboserve_value *url = keyptr;
    int r;
...
static enum turboserve_http_status add_script_paths(const struct private_data *pd,
...
static void add_header_to_strbuf(const char *header,
                                 ...
                                 void *user_data)
{
    struct turboserve_strbuf *strbuf = user_data;
    return add_param_len(strbuf, header, header_len, value, value_len);
}

static bool fill_addr_and_port(const struct turboserve_request *r,
                               struct turboserve_strbuf *strbuf)
{
    const struct turboserve_thread *t = r->conn->thread;
    ...
    remote_addr = turboserve_request_get_remote_address_and_port(r, remote_addr_buf,
                                                           &remote_port);
...
static enum turboserve_http_status add_params(const struct private_data *pd,
                                        struct turboserve_request *request,
                                        struct turboserve_response *response)
{
    const struct turboserve_request_parser_helper *request_helper = request->helper;
    struct turboserve_strbuf *strbuf = response->buffer;
    ...
    add_param(strbuf, "SERVER_SOFTWARE", "turboserve");
    add_param(strbuf, "SERVER_PROTOCOL",
              request->flags & REQUEST_IS_HTTP_1_0 ? "HTTP/1.0" : "HTTP/1.1");

    add_param(strbuf, "REQUEST_METHOD", turboserve_request_get_method_str(request));
    ...
    turboserve_request_foreach_header_for_cgi(request, add_header_to_strbuf, strbuf);

    return HTTP_OK;
}

static bool
handle_stdout(struct turboserve_request *request, const struct record *record, int fd)
{
    size_t to_read = record->len_content;
    char *buffer = turboserve_strbuf_extend_unsafe(request->response.buffer, to_read);

    if (!buffer)
        return false;

    while (to_read) {
        ssize_t r = turboserve_recv_fd(request, fd, buffer, to_read, 0);
    ...
    if (record->len_padding) {
        char padding[256];
        if (turboserve_recv_fd(request, fd, padding, (size_t)record->len_padding,
                         MSG_TRUNC) < 0) {
            return false;
        }
    }
...
static bool
handle_stderr(struct turboserve_request *request, const struct record *record, int fd)
{
    size_t to_read = record->len_content;
    ...
    for (char *p = buffer; to_read;) {
        ssize_t r = turboserve_recv_fd(request, fd, p, to_read, 0);
    ...
    turboserve_status_error("FastCGI stderr output: %.*s", (int)record->len_content,
                      buffer);
...
static bool discard_unknown_record(struct turboserve_request *request,
                                   ...
                                   int fd)
{
    ...
    turboserve_status_debug("Discarding record of type %d (%zu bytes incl. padding)",
                      record->type, to_read);

    while (to_read) {
        ssize_t r;

        r = turboserve_recv_fd(request, fd, buffer, turboserve_MIN(sizeof(buffer), to_read),
                         MSG_TRUNC);
        if (r < 0)
            return false;
...
DEFINE_ARRAY_TYPE_INLINEFIRST(header_array, struct turboserve_key_value)
...
try_initiating_chunked_response(struct turboserve_request *request)
{
    ...
    struct turboserve_response *response = &request->response;
    char *header_start[N_HEADER_START];
    char *next_request;
    enum turboserve_http_status status_code = HTTP_OK;
    struct turboserve_value buffer = turboserve_strbuf_to_value(response->buffer);
    ...
    ssize_t n_headers = turboserve_find_headers(header_start, &buffer, &next_request);
    if (n_headers < 0)
        ...
        *(end - 2) = '\0';
        ...
        if (strcaseequal_neutral(key, "X-Powered-By")) {
            ...
            if (status_as_int < 100 || status_as_int >= 600)
                ...
                status_code = (enum turboserve_http_status)status_as_int;
        } else {
            struct turboserve_key_value *header = header_array_append(&additional_headers);

            if (!header)
                goto free_array_and_disarm;

            *header = (struct turboserve_key_value){.key = key, .value = value};
        }
    }
    ...
    if (!header)
        goto free_array_and_disarm;
    *header = (struct turboserve_key_value){};

    if (!turboserve_response_set_chunked_full(request, status_code,
    ...
    if (chunk_len) {
        struct turboserve_strbuf first_chunk;

        turboserve_strbuf_init(&first_chunk);
        turboserve_strbuf_set_static(&first_chunk, chunk_start, chunk_len);
        turboserve_response_send_chunk_full(request, &first_chunk);

        /* We sent the chunk using a temporary strbuf, so reset the
         * actual buffer that had the headers+first chunk */
        turboserve_strbuf_reset(response->buffer);
    }
...
static bool build_stdin_records(struct turboserve_request *request,
                                struct iovec_array *iovec_array,
                                int fcgi_fd,
                                const struct turboserve_value *body_data)
{
    struct iovec *iovec;
    ...
    if (body_data) {
        ...
        while (to_send) {
            ...
            size_t block_size = turboserve_MIN(0xffffull, to_send);
            ...
            *iovec = (struct iovec){.iov_base = buffer, .iov_len = block_size};
            ...
            if (iovec_array_len(iovec_array) == turboserve_ARRAY_INCREMENT) {
                if (turboserve_writev_fd(request, fcgi_fd,
...
static enum turboserve_http_status send_request(struct private_data *pd,
                                          struct turboserve_request *request,
                                          struct turboserve_response *response,
                                          int fcgi_fd)
{
    ...
    if (UNLIKELY(turboserve_strbuf_get_length(response->buffer) > 0xffffu)) {
    ...
    *iovec = (struct iovec){
                ...
                .begin_params =
                    {
                        ...
                        .id = htons(1),
                        .len_content = htons(
                            (uint16_t)turboserve_strbuf_get_length(response->buffer)),
                    },
            },
    ...
    if (UNLIKELY(!iovec))
        return HTTP_INTERNAL_ERROR;
    *iovec = turboserve_strbuf_to_iovec(response->buffer);
    ...
    *iovec = (struct iovec){
    ...
    if (!build_stdin_records(request, &iovec_array, fcgi_fd,
                             turboserve_request_get_request_body(request))) {
        return HTTP_INTERNAL_ERROR;
    }

    if (turboserve_writev_fd(request, fcgi_fd, iovec_array_get_array(&iovec_array),
                       (int)iovec_array_len(&iovec_array)) < 0) {
        return HTTP_INTERNAL_ERROR;
    }
    iovec_array_reset(&iovec_array);
...
static bool try_connect(struct turboserve_request *request,
                        int sock_fd,
                        ...
                        socklen_t socklen)
{
    ...
    for (int try = 0; try < 10; try++) {
        ...
        turboserve_request_await_write(request, sock_fd);
...
static enum turboserve_http_status
fastcgi_handle_request(struct turboserve_request *request,
                       struct turboserve_response *response,
                       void *instance)
{
    ...
    turboserve_strbuf_append_char(response->buffer, '\r');
    ...
    while (true) {
        ...
        r = turboserve_recv_fd(request, fcgi_fd, &record, sizeof(record), 0);
        if (r < 0)
            return HTTP_UNAVAILABLE;
        ...
        case FASTCGI_TYPE_END_REQUEST:
            if (request->flags & RESPONSE_CHUNKED_ENCODING) {
                if (turboserve_strbuf_get_length(response->buffer) != 0) {
...
static void *fastcgi_create(const char *prefix __attribute__((unused)),
                            void *user_settings)
{
    struct turboserve_fastcgi_settings *settings = user_settings;
    struct private_data *pd;

    if (!settings->address) {
        turboserve_status_error("FastCGI: `address` not specified");
        return NULL;
    }
    if (!settings->script_path) {
        turboserve_status_error("FastCGI: `script_path` not specified");
        return NULL;
    }
    ...
    pd = malloc(sizeof(*pd));
    if (!pd) {
        turboserve_status_perror("FastCGI: Could not allocate memory for module");
        return NULL;
    }
    ...
    if (!pd->script_name_cache) {
        turboserve_status_error("FastCGI: could not create cache for script_name");
        goto free_pd;
    }

    pd->default_index = (struct turboserve_value){
        .value = strdup(settings->default_index),
        .len = strlen(settings->default_index),
    };
    if (!pd->default_index.value) {
        turboserve_status_error("FastCGI: could not copy default_address for module");
        goto destroy_cache;
    }

    pd->script_path = realpath(settings->script_path, NULL);
    if (!pd->script_path) {
        turboserve_status_perror("FastCGI: `script_path` of '%s' is invalid",
    ...
    if (pd->script_path_fd < 0) {
        turboserve_status_perror("FastCGI: Could not open `script_path` at '%s'",
                           pd->script_path);
    ...
    if (*settings->address == '/') {
        ...
        if (stat(settings->address, &st) < 0) {
            turboserve_status_perror("FastCGI: `address` not found: %s",
                               settings->address);
            goto close_script_path_fd;
        }

        if (!(st.st_mode & S_IFSOCK)) {
            turboserve_status_error("FastCGI: `address` is not a socket: %s",
                              settings->address);
            goto close_script_path_fd;
        }

        if (strlen(settings->address) >= sizeof(pd->un_addr.sun_path)) {
            turboserve_status_error(
    ...
    pd->addr_family = turboserve_socket_parse_address(address_copy, &node, &port);

    int int_port = parse_int(port, -1);
    if (int_port < 0 || int_port > 0xffff) {
        turboserve_status_error("FastCGI: Port %d is not in valid range [0-65535]",
                          int_port);
        goto free_address_copy;
    }
    ...
    case AF_MAX:
        turboserve_status_error("FastCGI: Could not parse '%s' as 'address:port'",
                          settings->address);
        goto free_address_copy;
    ...
    case AF_INET: {
        ...
        if (inet_pton(AF_INET, node, &in_addr) < 0) {
            turboserve_status_perror("FastCGI: Could not parse IPv4 address '%s'",
                               node);
            goto free_address_copy;
        }
    ...
    case AF_INET6: {
        ...
        if (inet_pton(AF_INET6, node, &in6_addr) < 0) {
            turboserve_status_perror("FastCGI: Could not parse IPv6 address '%s'",
                               node);
            goto free_address_copy;
        }
    ...
    turboserve_status_error("FastCGI: Address '%s' isn't a valid Unix Domain Socket, IPv4, or IPv6 address",
                      settings->address);
...
{
    struct turboserve_fastcgi_settings settings = {
...
turboserve_REGISTER_MODULE(fastcgi, &module);
