...
redirect_handle_request(struct turboserve_request *request,
                        struct turboserve_response *response,
                        void *instance)
{
    struct redirect_priv *priv = instance;
    struct turboserve_key_value headers[] = {{"Location", priv->to}, {}};
...
static void *redirect_create(const char *prefix __attribute__((unused)),
                             void *instance)
{
    struct turboserve_redirect_settings *settings = instance;
...
static enum turboserve_http_status parse_http_code(const char *code,
                                             enum turboserve_http_status fallback)
{
    const char *known;
    int as_int;
    ...
    known = turboserve_http_status_as_string_with_code((enum turboserve_http_status)as_int);
    if (!strncmp(known, "999", 3))
        return fallback;

    return (enum turboserve_http_status)as_int;
}

static void *redirect_create_from_hash(const char *prefix,
                                       const struct hash *hash)
{
    struct turboserve_redirect_settings settings = {
...
turboserve_REGISTER_MODULE(redirect, &module);
