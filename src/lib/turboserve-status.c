...
void turboserve_status_shutdown(struct turboserve *l __attribute__((unused))) {}
...
static int status_index(enum turboserve_status_type type)
{
    return use_colors ? (int)type : STATUS_NONE;
}

#define V(c) { .value = c, .len = sizeof(c) - 1 }
static const struct turboserve_value start_colors[] = {
...
static inline struct turboserve_value start_color(enum turboserve_status_type type)
{
    return start_colors[status_index(type)];
}

static inline struct turboserve_value end_color(void)
{
    return use_colors ? (struct turboserve_value)V("\033[0m\n")
                      : (struct turboserve_value)V("\n");
}
#undef V
...
void turboserve_syslog_status_out(
#ifndef NDEBUG
    const char *file,
    const int line,
    const char *func,
    const long tid,
#endif
    enum turboserve_status_type type,
    int saved_errno,
    const char *fmt,
    va_list values)
{
    ...
    turboserve_strbuf_init_with_fixed_buffer(&buf, syslog_buffer,
                                       sizeof(syslog_buffer));

#ifndef NDEBUG
    if (!turboserve_strbuf_append_printf(&buf, "%ld %s:%d %s() ", tid,
                                   basename(strdupa(file)), line, func))
        goto out;
#endif

    va_copy(copied_values, values);
    if (!turboserve_strbuf_append_vprintf(&buf, fmt, copied_values))
        goto out;

    if (type & STATUS_PERROR) {
        char errbuf[128];

        if (!turboserve_strbuf_append_strz(
                &buf,
    ...
    syslog(status_to_syslog_prio[type], "%.*s",
           (int)turboserve_strbuf_get_length(&buf), turboserve_strbuf_get_buffer(&buf));

out:
    turboserve_strbuf_free(&buf);
}

turboserve_CONSTRUCTOR(register_turboserve_to_syslog, 0)
{
    openlog("turboserve", LOG_NDELAY | LOG_PID | LOG_CONS, LOG_USER);
}
#else
#define turboserve_syslog_status_out(...)
#endif
...
static void status_out(
    ...
    va_list values)
{
    struct turboserve_value start = start_color(type);
    struct turboserve_value end = end_color();
    int saved_errno = errno;

#ifndef NDEBUG
    turboserve_syslog_status_out(file, line, func, gettid_cached(), type, saved_errno,
                           fmt, values);
#else
    turboserve_syslog_status_out(type, saved_errno, fmt, values);
#endif
...
#define IMPLEMENT_FUNCTION(fn_name_, type_)                                    \
    void turboserve_status_##fn_name_(const char *fmt, ...)                          \
    {                                                                          \
        if (LIKELY(!quiet)) {                                                  \
            va_list values;                                                    \
            va_start(values, fmt);                                             \
...
#else
#define IMPLEMENT_FUNCTION(fn_name_, type_)                                    \
    void turboserve_status_##fn_name_##_debug(const char *file, const int line,      \
