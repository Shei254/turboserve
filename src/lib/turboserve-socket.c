...
static int set_socket_flags(int fd)
{
    ...
    if (flags < 0)
        turboserve_status_critical_perror("Could not obtain socket flags");
    if (fcntl(fd, F_SETFD, flags | FD_CLOEXEC) < 0)
        turboserve_status_critical_perror("Could not set socket flags");
                           ...
                           bool is_https)
{
    if (listen(fd, get_backlog_size()) < 0)
        turboserve_status_critical_perror("listen");
    ...
    if (print_listening_msg) {
        ...
        if (ret)
            turboserve_status_critical("getnameinfo: %s", gai_strerror(ret));

        if (addr->ai_family == AF_INET6)
            turboserve_status_info("Listening on http%s://[%s]:%s", s_if_https,
                             host_buf, serv_buf);
        else
            turboserve_status_info("Listening on http%s://%s:%s", s_if_https,
                             host_buf, serv_buf);
...
#define SET_SOCKET_OPTION(_domain, _option, _param)                            \
    do {                                                                       \
        ...
        if (setsockopt(fd, (_domain), (_option), (_param), _param_size_) < 0)  \
            turboserve_status_critical_perror("setsockopt");                         \
    } while (0)
...
#define SET_SOCKET_OPTION_MAY_FAIL(_domain, _option, _param)                   \
    do {                                                                       \
        const socklen_t _param_size_ = (socklen_t)sizeof(*(_param));           \
        if (setsockopt(fd, (_domain), (_option), (_param), _param_size_) < 0)  \
            turboserve_status_perror("%s not supported by the kernel", #_option);    \
    } while (0)
                                     ...
                                     bool is_https)
{
...
#else
        turboserve_status_critical("SO_REUSEPORT not supported by the OS");
#endif
    ...
    turboserve_status_critical("Could not bind socket");
}

static int set_socket_options(const struct turboserve *l, int fd)
{
...
static int setup_socket_normally(const struct turboserve *l,
                                 bool print_listening_msg,
                                 bool is_https,
                                 const char *listener_from_config)
{
    ...
    sa_family_t family = turboserve_socket_parse_address(listener, &node, &port);

    if (family == AF_MAX) {
        turboserve_status_critical("Could not parse listener: %s",
                             l->config.listener);
    }
    ...
    if (ret)
        turboserve_status_critical("getaddrinfo: %s", gai_strerror(ret));
...
static int from_systemd_socket(const struct turboserve *l, int fd)
{
    if (!sd_is_socket_inet(fd, AF_UNSPEC, SOCK_STREAM, 1, 0)) {
        turboserve_status_critical("Passed file descriptor is not a "
                              ...
                              bool is_https)
{
    ...
    if (!strncmp(listener, "systemd:", sizeof("systemd:") - 1)) {
        ...
        if (n < 0) {
            errno = -n;
            turboserve_status_perror(
                "Could not parse socket activation data from systemd");
            return n;
        }

        if (n == 0) {
            turboserve_status_critical("Not invoked from systemd "
    ...
    if (streq(listener, "systemd")) {
        ...
        if (n < 0) {
            errno = -n;
            turboserve_status_perror("Could not obtain sockets passed from systemd");
            return n;
        }
