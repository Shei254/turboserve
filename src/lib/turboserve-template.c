...
struct parser {
    ...
    struct chunk_array chunks;
    enum turboserve_tpl_flag template_flags;
};
...
struct chunk_descriptor {
    struct chunk *chunk;
    struct turboserve_var_descriptor *descriptor;
};
...
static struct turboserve_var_descriptor *symtab_lookup(struct parser *parser,
                                                 const char *var_name)
{
    for (struct symtab *tab = parser->symtab; tab; tab = tab->next) {
        struct turboserve_var_descriptor *var = hash_find(tab->hash, var_name);
        if (var)
            return var;
    }
...
static __attribute__((noinline)) struct turboserve_var_descriptor *
symtab_lookup_lexeme(struct parser *parser, struct lexeme *lexeme)
{
    if (lexeme->value.len > LEXEME_MAX_LEN) {
        turboserve_status_error("Lexeme exceeds %d characters", LEXEME_MAX_LEN);
        return NULL;
    }
...
static void error_vlexeme(struct lexeme *lexeme, const char *msg, va_list ap)
{
    ...
    *lexeme = (struct lexeme){.type = LEXEME_ERROR};
    ...
    turboserve_status_error("Error while parsing template: %.*s", (int)formatted_len, formatted);
    free(formatted);
}
...
static void lex_init(struct lexer *lexer, struct turboserve_value input)
{
...
static void parser_push_lexeme(struct parser *parser, struct lexeme *lexeme)
{
    ...
    if (!stacked_lexeme)
        turboserve_status_critical_perror("Could not push parser lexeme");
                       ...
                       void *data)
{
    ...
    if (!chunk)
        turboserve_status_critical_perror("Could not emit template chunk");
...
{
    struct chunk *iter;
    struct turboserve_var_descriptor *symbol;
    ...
    turboserve_ARRAY_FOREACH_REVERSE(&parser->chunks, iter) {
...
static void *parser_partial(struct parser *parser, struct lexeme *lexeme)
{
    ...
    tpl = turboserve_tpl_compile_file(filename, parser->descriptor);
    if (tpl) {
...
static struct turboserve_strbuf *turboserve_strbuf_from_lexeme(struct parser *parser,
                                                   struct lexeme *lexeme)
{
    if (parser->template_flags & turboserve_TPL_FLAG_CONST_TEMPLATE)
        return turboserve_strbuf_new_static(lexeme->value.value, lexeme->value.len);

    struct turboserve_strbuf *buf = turboserve_strbuf_new_with_size(lexeme->value.len);
    if (buf)
        turboserve_strbuf_set(buf, lexeme->value.value, lexeme->value.len);
...
static void *parser_text(struct parser *parser, struct lexeme *lexeme)
{
    ...
    if (lexeme->type == LEXEME_TEXT) {
        if (lexeme->value.len > 4 * sizeof(void *)) {
            struct turboserve_strbuf *buf = turboserve_strbuf_from_lexeme(parser, lexeme);
            if (!buf)
            ...
            while (copy.value.len) {
                const size_t len = turboserve_MIN(sizeof(void *), copy.value.len);
                uintptr_t tmp = 0;
...
void turboserve_append_int_to_strbuf(struct turboserve_strbuf *buf, void *ptr)
{
    ...
    turboserve_strbuf_append_str(buf, converted, len);
}

bool turboserve_tpl_int_is_empty(void *ptr) { return (*(int *)ptr) == 0; }

void turboserve_append_double_to_strbuf(struct turboserve_strbuf *buf, void *ptr)
{
    turboserve_strbuf_append_printf(buf, "%f", *(double *)ptr);
}

bool turboserve_tpl_double_is_empty(void *ptr)
{
#if defined(turboserve_HAVE_BUILTIN_FPCLASSIFY)
...
void turboserve_append_str_to_strbuf(struct turboserve_strbuf *buf, void *ptr)
{
    ...
    if (LIKELY(str))
        turboserve_strbuf_append_strz(buf, str);
}
...
{
    static const struct turboserve_value escaped[] = {
        {},
        { /* / */ "&#x2f;", 6 },
        { /* ' */ "&#x27;", 6 },
        { /* " */ "&quot;", 6 },
    ...
    for (last = p = str; *p; p++) {
        ...
        if (index) {
            turboserve_strbuf_append_str(buf, last, (size_t)(p - last));
            last = p + 1;

            turboserve_strbuf_append_str(buf, escaped[index].value, escaped[index].len);
        }
    }
...
static void
unbake_direct_addresses(struct turboserve_tpl *tpl)
{
    struct chunk *iter;
    ...
    turboserve_ARRAY_FOREACH (&tpl->chunks, iter) {
...
static void free_chunk(struct chunk *chunk)
{
    ...
    case ACTION_START_ITER:
        ...
        break;
    case ACTION_APPEND:
        turboserve_strbuf_free(chunk->data);
        break;
    case ACTION_APPLY_TPL:
        turboserve_tpl_free(chunk->data);
        break;
    }
}
...
static void free_chunk_array(struct chunk_array *array)
{
    ...
    turboserve_ARRAY_FOREACH(array, iter)
        free_chunk(iter);
...
static bool post_process_template(struct parser *parser)
{
    ...
    turboserve_ARRAY_FOREACH (&parser->chunks, chunk) {
        if (chunk->action == ACTION_IF_VARIABLE_NOT_EMPTY) {
            for (prev_chunk = chunk;; chunk++) {
                if (chunk == last_chunk)
                    goto error;
                if (chunk->action == ACTION_LAST) {
                    turboserve_status_error("Internal error: Could not find the end "
                                      "var not empty chunk");
                    return false;
                }
            ...
            struct turboserve_var_descriptor *descriptor = chunk->data;
            bool escape = chunk->flags & FLAGS_QUOTE;

            if (descriptor->append_to_strbuf == turboserve_append_str_to_strbuf) {
                if (escape)
                ...
                turboserve_status_error("Variable must be string to be escaped");
                return false;
            } else if (!descriptor->append_to_strbuf) {
                turboserve_status_error("Invalid variable descriptor");
                return false;
            }
...
error:
    turboserve_status_error("Unknown error while parsing template; bug?");
    return false;
}
...
static bool parser_shutdown(struct parser *parser, struct lexeme *lexeme)
{
    ...
    if (lexeme && lexeme->type == LEXEME_ERROR && lexeme->value.value) {
        turboserve_status_error("Parser error: %.*s", (int)lexeme->value.len,
                          lexeme->value.value);
    ...
    if (parser->flags & FLAGS_NEGATE) {
        turboserve_status_error("Parser error: unmatched negation");
        success = false;
    }
    if (parser->flags & FLAGS_QUOTE) {
        turboserve_status_error("Parser error: unmatched quote");
        success = false;
    }
...
static bool parse_value(struct turboserve_tpl *tpl,
                        struct turboserve_value value,
                        const struct turboserve_var_descriptor *descriptor,
...
static void dump_program(const struct turboserve_tpl *tpl)
{
    struct chunk *iter;
    int indent = 0;
    ...
    turboserve_ARRAY_FOREACH(&tpl->chunks, iter) {
        ...
        case ACTION_APPEND:
            printf("%s [%.*s]", instr("APPEND", instr_buf),
                   (int)turboserve_strbuf_get_length(iter->data),
                   turboserve_strbuf_get_buffer(iter->data));
            break;
        case ACTION_APPEND_SMALL: {
        ...
        case ACTION_LAMBDA: {
            struct turboserve_var_descriptor *descriptor = iter->data;
...
turboserve_tpl_compile_string(const char *string,
                        const struct turboserve_var_descriptor *descriptor)
{
    return turboserve_tpl_compile_string_full(string, descriptor, 0);
}
                      ...
                      const struct turboserve_var_descriptor *descriptor)
{
    ...
    struct stat st;
    char *mapped;
    struct turboserve_tpl *tpl = NULL;
    ...
    tpl = turboserve_tpl_compile_string(mapped, descriptor);

    if (munmap(mapped, (size_t)st.st_size) < 0)
        turboserve_status_perror("munmap");
...
static void
bake_direct_addresses(struct turboserve_tpl *tpl,
...
static const struct chunk *apply(struct turboserve_tpl *tpl,
...
#define RETURN_IF_NO_CHUNK(force_)                                             \
    do {                                                                       \
        if (force_ UNLIKELY(!chunk)) {                                         \
            turboserve_status_error("Chunk is NULL while dispatching");              \
            return NULL;                                                       \
        }                                                                      \
    } while (false)
...
action_append:
    turboserve_strbuf_append_str(buf, turboserve_strbuf_get_buffer(chunk->data),
                           turboserve_strbuf_get_length(chunk->data));
...
action_append_small: {
        ...
        turboserve_strbuf_append_str(buf, (char*)&val, len);
...
action_variable_str:
    turboserve_append_str_to_strbuf(buf, (char *)variables + (uintptr_t)chunk->data);
    DISPATCH_NEXT_ACTION_FAST();

action_variable_str_escape:
    turboserve_append_str_escaped_to_strbuf(buf, (char *)variables +
...
action_if_variable_not_empty: {
        ...
        if (chunk->flags & FLAGS_LAMBDA) {
            struct turboserve_strbuf temp_buf;
            char trash[1];

            turboserve_strbuf_init_with_fixed_buffer(&temp_buf, trash, 0);

            cd->descriptor->lambda(&temp_buf, variables);
            empty = turboserve_strbuf_has_grow_buffer_failed_flag(&temp_buf);

            turboserve_strbuf_free(&temp_buf);
        } else {
...
action_apply_tpl: {
        struct turboserve_tpl *inner_tpl = chunk->data;

        if (LIKELY(turboserve_strbuf_grow_by(buf, inner_tpl->minimum_size))) {
            if (!apply(inner_tpl, chunk_array_get_array(&inner_tpl->chunks),
                       buf, variables, NULL)) {
                turboserve_status_warning("Could not apply subtemplate");
                return NULL;
            }
        } else {
            turboserve_status_warning("Could not grow template by %zu bytes",
...
action_start_iter:
    if (UNLIKELY(coro != NULL)) {
        turboserve_status_warning("Coroutine is not NULL when starting iteration");
        return NULL;
    }
...
action_end_iter:
    ...
    if (UNLIKELY(!coro)) {
        if (!chunk->flags) {
            turboserve_status_warning("Coroutine is NULL when finishing iteration");
            return NULL;
        }
        DISPATCH_NEXT_ACTION_FAST();
    }
...
bool turboserve_tpl_apply_with_buffer(struct turboserve_tpl *tpl,
                                ...
                                void *variables)
{
    turboserve_strbuf_reset(buf);

    if (UNLIKELY(!turboserve_strbuf_grow_to(buf, tpl->minimum_size)))
        return false;
...
{
    struct turboserve_strbuf *buf = turboserve_strbuf_new_with_size(tpl->minimum_size);

    if (UNLIKELY(!buf))
        return NULL;

    if (LIKELY(turboserve_tpl_apply_with_buffer(tpl, buf, variables)))
        return buf;

    turboserve_strbuf_free(buf);
    return NULL;
}
...
static void hello_lambda(struct turboserve_strbuf *b, void *v)
{
    struct test_struct *s = v;

    turboserve_strbuf_append_printf("hello from lambda func! appending <%d> from test_struct", s->some_int);
}
...
int main(int argc, char *argv[])
{
    ...
    struct turboserve_var_descriptor desc[] = {
        TPL_VAR_INT(some_int),
        TPL_VAR_STR(a_string),
        TPL_LAMBDA(hello, hello_lambda),
        TPL_VAR_SENTINEL
    };
    struct turboserve_tpl *tpl = turboserve_tpl_compile_file(argv[1], desc);
    if (!tpl)
        return 1;

    printf("*** Applying template 10000000 times...\n");
    for (size_t i = 0; i < 10000000; i++) {
        struct turboserve_strbuf *applied = turboserve_tpl_apply(tpl, &(struct test_struct) {
            .some_int = 42,
            .a_string = "some string"
        });
        turboserve_strbuf_free(applied);
    }
