...
struct turboserve_serve_files_settings {
...
turboserve_MODULE_FORWARD_DECL(serve_files);

#define SERVE_FILES_SETTINGS(root_path_, index_html_, serve_precompressed_files_) \
  .module = turboserve_MODULE_REF(serve_files), \
  .args = ((struct turboserve_serve_files_settings[]) {{ \
  ...
  .flags = (enum turboserve_handler_flags)0
