...
static bool get_user_uid_gid(const char *user, uid_t *uid, gid_t *gid)
{
    ...
    if (!buf) {
        turboserve_status_error("Could not allocate buffer for passwd struct");
        return false;
    }
    ...
    *gid = pwd.pw_gid;
    ...
    if (!r) {
        turboserve_status_error("Username not found: %s", user);
    } else {
        errno = r;
        turboserve_status_perror("Could not obtain uid/gid for user %s", user);
    }
...
static bool switch_to_user(uid_t uid, gid_t gid, const char *username)
{
    ...
    turboserve_status_info("Dropping privileges to UID %d, GID %d (%s)", uid, gid,
                     username);
...
static void abort_on_open_directories(void)
{
     ...
     * should be fine (maybe not if turboserve is used as a library.)
     */
    ...
    if (ret < 0 || ret >= (int)sizeof(own_fd)) {
        turboserve_status_critical("Could not get descriptor of /proc/self/fd");
    }
    ...
    while ((ent = readdir(dir))) {
        ...
        if (len < 0) {
            turboserve_status_critical_perror("Could not get information about fd %s",
                                        ent->d_name);
        }
        path[len] = '\0';
...
void turboserve_straitjacket_enforce(const struct turboserve_straitjacket *sj)
{
    ...
    if (geteuid() != 0)
        turboserve_status_critical("Straitjacket requires root privileges");

    if (sj->user_name && *sj->user_name) {
        if (!get_user_uid_gid(sj->user_name, &uid, &gid))
            turboserve_status_critical("Unknown user: %s", sj->user_name);
        got_uid_gid = true;
    }
    ...
    if (sj->chroot_path) {
        ...
        if (chroot(sj->chroot_path) < 0) {
            turboserve_status_critical_perror("Could not chroot() to %s",
                                        sj->chroot_path);
        }

        if (chdir("/") < 0)
            turboserve_status_critical_perror("Could not chdir() to /");

        turboserve_status_info("Jailed to %s", sj->chroot_path);
    }

    if (got_uid_gid && !switch_to_user(uid, gid, sj->user_name)) {
        turboserve_status_critical("Could not drop privileges to %s, aborting",
                             sj->user_name);
    }
...
out:
    if (sj->drop_capabilities) {
        ...
        if (capset(&header, &data) < 0)
            turboserve_status_critical_perror("Could not drop capabilities");
    }
}
...
void turboserve_straitjacket_enforce_from_config(struct config *c)
{
    ...
    while ((l = config_read_line(c))) {
        ...
        case CONFIG_LINE_TYPE_SECTION_END:
            turboserve_straitjacket_enforce(&(struct turboserve_straitjacket){
                .user_name = user_name,
