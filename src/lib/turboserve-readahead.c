...
struct turboserve_readahead_cmd {
...
#ifdef _SC_PAGESIZE
turboserve_CONSTRUCTOR(get_page_size, 0)
{
...
{
    struct turboserve_readahead_cmd cmd = {
        .cmd = SHUTDOWN,
    };
    ...
    turboserve_status_debug("Shutting down readahead thread");
...
void turboserve_madvise_queue(void *addr, size_t length)
{
...
static void *turboserve_readahead_loop(void *data __attribute__((unused)))
{
    ...
    turboserve_set_thread_name("readahead");

    while (true) {
        struct turboserve_readahead_cmd cmd[16];
        ...
        if (UNLIKELY(n_bytes < 0)) {
            if (errno == EAGAIN || errno == EINTR)
                continue;
            turboserve_status_perror("Ignoring error while reading from pipe (%d)",
                               readahead_pipe_fd[0]);
            continue;
#if PIPE_DIRECT_FLAG
        } else if (UNLIKELY(n_bytes % (ssize_t)sizeof(cmd[0]))) {
            turboserve_status_warning("Ignoring readahead packet read of %zd bytes",
                                n_bytes);
            continue;
#endif
        }

        cmds = n_bytes / (ssize_t)sizeof(struct turboserve_readahead_cmd);
...
void turboserve_readahead_init(void)
{
    ...
    turboserve_status_debug("Initializing low priority readahead thread");

    if (pipe2(readahead_pipe_fd, O_CLOEXEC | PIPE_DIRECT_FLAG) < 0) {
        turboserve_status_warning("Could not create pipe for readahead queue");
    ...
    if (pthread_create(&readahead_self, NULL, turboserve_readahead_loop, NULL)) {
        turboserve_status_warning("Could not create low-priority readahead thread");
        goto disable_readahead_close_pipe;
    }
...
disable_readahead:
    ...
    turboserve_status_warning("Readahead thread has been disabled");
}
