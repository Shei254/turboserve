...
#include "turboserve-mod-rewrite.h"
...
enum pattern_flag {
    ...
    PATTERN_EXPAND_turboserve = 1 << 2,
    PATTERN_EXPAND_LUA = 1 << 3,
    PATTERN_EXPAND_MASK = PATTERN_EXPAND_turboserve | PATTERN_EXPAND_LUA,
...
struct pattern {
    ...
    char *expand_pattern;
    struct {
        struct turboserve_key_value cookie;
        struct turboserve_key_value env_var;
        struct turboserve_key_value query_var;
        struct turboserve_key_value post_var;
        struct turboserve_key_value header;
        struct {
            char *path;
        } stat;
        struct {
...
static enum turboserve_http_status module_redirect_to(struct turboserve_request *request,
                                                const char *url)
{
...
static enum turboserve_http_status module_rewrite_as(struct turboserve_request *request,
                                               const char *url)
{
                                 ...
                                 int captures)
{
    ...
    if (!turboserve_strbuf_init_with_fixed_buffer(&strbuf, buffer, PATH_MAX))
        return NULL;
    ...
    do {
        ...
        if (ptr > expand_pattern) {
            ...
            if (UNLIKELY(!turboserve_strbuf_append_str(&strbuf, expand_pattern, len)))
                return NULL;
        ...
        if (LIKELY(index_len > 0)) {
            ...
            if (UNLIKELY(!turboserve_strbuf_append_str(
                    &strbuf, orig + sf[index].sm_so,
                    (size_t)(sf[index].sm_eo - sf[index].sm_so))))
                return NULL;

            expand_pattern += index_len;
        } else if (UNLIKELY(!turboserve_strbuf_append_char(&strbuf, '%'))) {
            return NULL;
        }
    ...
    if (remaining_len &&
        !turboserve_strbuf_append_str(&strbuf, expand_pattern, remaining_len))
        return NULL;

    if (UNLIKELY(!turboserve_strbuf_get_length(&strbuf)))
        return NULL;

    return turboserve_strbuf_get_buffer(&strbuf);
}
...
static const char *expand_lua(struct turboserve_request *request,
                              ...
                              const struct str_find *sf, int captures)
{
    ...
    L = turboserve_lua_create_state(NULL, pattern->expand_pattern);
    if (UNLIKELY(!L))
        return NULL;
    ...
    if (!lua_isfunction(L, -1)) {
        turboserve_status_error(
            ...
            turboserve_lua_state_last_error(L));
        return NULL;
    }

    turboserve_lua_state_push_request(L, request);
    ...
    if (lua_pcall(L, 2, 1, 0) != 0) {
        turboserve_status_error("Could not execute `handle_rewrite()` function: %s",
                          turboserve_lua_state_last_error(L));
        return NULL;
    }
    ...
    if (output_len >= PATH_MAX) {
        turboserve_status_error("Rewritten URL exceeds %d bytes (got %zu bytes)",
                          PATH_MAX, output_len);
        return NULL;
    }
...
static bool condition_matches(struct turboserve_request *request,
                              ...
                              char expanded_buf[static PATH_MAX])
{
    ...
    if (p->flags & PATTERN_COND_METHOD) {
        const enum turboserve_request_flags method =
            p->condition.request_flags & REQUEST_METHOD_MASK;
        if (turboserve_request_get_method(request) != method)
            return false;
    }
    ...
    if (p->flags & PATTERN_COND_ACCEPT_ENCODING) {
        const enum turboserve_request_flags accept =
            p->condition.request_flags & REQUEST_ACCEPT_MASK;
        if (!(turboserve_request_get_accept_encoding(request) & accept))
            return false;
    }
    ...
    if (p->flags & PATTERN_COND_COOKIE) {
        ...
        const char *cookie =
            turboserve_request_get_cookie(request, p->condition.cookie.key);
        if (!cookie)
            return false;
    ...
    if (p->flags & PATTERN_COND_QUERY_VAR) {
        ...
        const char *query =
            turboserve_request_get_query_param(request, p->condition.query_var.key);
    ...
    if (p->flags & PATTERN_COND_POST_VAR) {
        ...
        const char *post =
            turboserve_request_get_post_param(request, p->condition.post_var.key);
        if (!post)
            return false;
    ...
    if (p->flags & PATTERN_COND_LUA) {
        ...
        lua_State *L = turboserve_lua_create_state(NULL, p->condition.lua.script);
        if (!L)
            return false;
        coro_defer(request->conn->coro, lua_close_defer, L);
        ...
        if (lua_pcall(L, 1, 1, 0) != 0) {
            turboserve_status_error("Could not execute `matches()` function: %s",
                              turboserve_lua_state_last_error(L));
            return false;
        }
...
rewrite_handle_request(struct turboserve_request *request,
                       ...
                       void *instance)
{
    ...
    turboserve_ARRAY_FOREACH(&pd->patterns, p) {
        ...
        case PATTERN_EXPAND_LUA:
            ...
            break;
#endif
        case PATTERN_EXPAND_turboserve:
...
static void rewrite_destroy(void *instance)
{
    ...
    turboserve_ARRAY_FOREACH(&pd->patterns, iter) {
        free(iter->pattern);
...
static void parse_condition_key_value(struct pattern *pattern,
                                      struct turboserve_key_value *key_value,
                                      ...
                                      const struct config_line *line)
{
    ...
    while ((line = config_read_line(config))) {
        ...
        case CONFIG_LINE_TYPE_SECTION_END:
            ...
            *key_value = (struct turboserve_key_value){key, value};
        ...
        case CONFIG_LINE_TYPE_LINE:
            ...
            if (streq(line->key, "condition_lua")) {
                ...
                if (!pattern->condition.lua.script)
                    turboserve_status_critical("Couldn't copy Lua script");
...
#else
                config_error(config, "turboserve has been built without Lua. "
                                     "`expand_with_lua` is not available");
                goto out;
#endif
            } else {
                pattern->flags |= PATTERN_EXPAND_turboserve;
            }
...
turboserve_REGISTER_MODULE(rewrite, &module);
