...
static struct turboserve_tpl *error_template = NULL;
...
#define TPL_STRUCT struct error_template
    static const struct turboserve_var_descriptor error_descriptor[] = {
        TPL_VAR_STR(short_message), TPL_VAR_STR(long_message),
        TPL_VAR_SENTINEL};

    assert(!error_template);

    turboserve_status_debug("Initializing default response");

    if (l->config.error_template) {
        error_template =
            turboserve_tpl_compile_file(l->config.error_template, error_descriptor);
    } else {
        error_template = turboserve_tpl_compile_value_full(
            response_template_value, error_descriptor, turboserve_TPL_FLAG_CONST_TEMPLATE);
    }

    if (UNLIKELY(!error_template))
        turboserve_status_critical_perror("turboserve_tpl_compile_string");
}

void turboserve_response_shutdown(struct turboserve *l __attribute__((unused)))
{
    turboserve_status_debug("Shutting down response");
    assert(error_template);
    turboserve_tpl_free(error_template);
}

static inline bool has_response_body(enum turboserve_request_flags method,
                                     enum turboserve_http_status status)
{
    /* See FOR_EACH_REQUEST_METHOD() in turboserve.h */
...
{
    const struct turboserve_response *response = &request->response;
    ...
    if (UNLIKELY(request->flags & RESPONSE_SENT_HEADERS)) {
        turboserve_status_debug("Headers already sent, ignoring call");
        return;
    }

    if (UNLIKELY(!response->mime_type)) {
        /* Requests without a MIME Type are errors from handlers that should
           just be handled by turboserve_default_response().  */
        return turboserve_default_response(request, status);
    }
    ...
    size_t header_len =
        turboserve_prepare_response_header(request, status, headers, sizeof(headers));
    if (UNLIKELY(!header_len))
        return turboserve_default_response(request, HTTP_INTERNAL_ERROR);

    if (!has_response_body(turboserve_request_get_method(request), status))
        return (void)turboserve_send(request, headers, header_len, 0);

    char *resp_buf = turboserve_strbuf_get_buffer(response->buffer);
    const size_t resp_len = turboserve_strbuf_get_length(response->buffer);
    if (sizeof(headers) - header_len > resp_len) {
        ...
        return (void)turboserve_send(request, headers, header_len + resp_len, 0);
    }
    ...
    return (void)turboserve_writev(request, response_vec, N_ELEMENTS(response_vec));
}

void turboserve_fill_default_response(struct turboserve_strbuf *buffer,
                                enum turboserve_http_status status)
{
    turboserve_tpl_apply_with_buffer(
        error_template, buffer,
        &(struct error_template){
            .short_message = turboserve_http_status_as_string(status),
            .long_message = turboserve_http_status_as_descriptive_string(status),
        });
}
                           ...
                           enum turboserve_http_status status)
{
    ...
    turboserve_fill_default_response(request->response.buffer, status);
...
flags_has_content_length(enum turboserve_request_flags v)
{
...
static ALWAYS_INLINE __attribute__((const)) bool
has_uncommon_response_headers(enum turboserve_request_flags v)
{
    return v & (RESPONSE_INCLUDE_REQUEST_ID | REQUEST_ALLOW_CORS |
                RESPONSE_CHUNKED_ENCODING | REQUEST_WANTS_HSTS_HEADER);
}

size_t turboserve_prepare_response_header_full(
    ...
    const struct turboserve_key_value *additional_headers)
{
     ...
     * can_override_header() in turboserve.c */

    char *p_headers;
    char *p_headers_end = headers + headers_buf_size;
    char buffer[INT_TO_STR_BUFFER_SIZE];
    const enum turboserve_request_flags request_flags = request->flags;
    const enum turboserve_connection_flags conn_flags = request->conn->flags;
    ...
    APPEND_STRING(turboserve_http_status_as_string_with_code(status));
...
skip_date_header:
    ...
    if (LIKELY(has_content_length)) {
        APPEND_CONSTANT("\r\nContent-Length: ");
        APPEND_UINT(turboserve_strbuf_get_length(request->response.buffer));
    }
    if (UNLIKELY(has_uncommon_response_headers(request_flags))) {
        ...
        if (request_flags & RESPONSE_INCLUDE_REQUEST_ID) {
            ...
            uint64_t id = turboserve_request_get_id(request);
...
ALWAYS_INLINE size_t turboserve_prepare_response_header(struct turboserve_request *request,
                                                  enum turboserve_http_status status,
                                                  char headers[],
                                                  size_t headers_buf_size)
{
    return turboserve_prepare_response_header_full(
                                    ...
                                    const struct turboserve_key_value *additional_headers)
{
    ...
    buffer_len = turboserve_prepare_response_header_full(request, status, buffer,
                                                   DEFAULT_BUFFER_SIZE,
                                                   additional_headers);
    ...
    turboserve_send(request, buffer, buffer_len, MSG_MORE);

    return true;
}

inline bool turboserve_response_set_chunked(struct turboserve_request *request,
                                      enum turboserve_http_status status)
{
    return turboserve_response_set_chunked_full(request, status,
...
{
    if (!(request->flags & RESPONSE_SENT_HEADERS)) {
        if (UNLIKELY(!turboserve_response_set_chunked(request, HTTP_OK)))
            return;
    }

    size_t buffer_len = turboserve_strbuf_get_length(strbuf);
    if (UNLIKELY(!buffer_len)) {
        static const char last_chunk[] = "0\r\n\r\n";
        turboserve_send(request, last_chunk, sizeof(last_chunk) - 1, 0);
        return;
    }
    ...
    struct iovec chunk_vec[] = {
        ...
        {.iov_base = turboserve_strbuf_get_buffer(strbuf), .iov_len = buffer_len},
        {.iov_base = "\r\n", .iov_len = 2},
    };

    turboserve_writev(request, chunk_vec, N_ELEMENTS(chunk_vec));

    turboserve_strbuf_reset(strbuf);
}

void turboserve_response_send_chunk(struct turboserve_request *request)
{
    return turboserve_response_send_chunk_full(request, request->response.buffer);
}
                                    ...
                                    enum turboserve_http_status status)
{
    ...
    buffer_len = turboserve_prepare_response_header(request, status, buffer,
                                              DEFAULT_BUFFER_SIZE);
    if (UNLIKELY(!buffer_len))
        return false;
...
void turboserve_response_send_event(struct turboserve_request *request, const char *event)
{
    ...
    if (!(request->flags & RESPONSE_SENT_HEADERS)) {
        if (UNLIKELY(!turboserve_response_set_event_stream(request, HTTP_OK)))
            return;
    }
    ...
    size_t buffer_len = turboserve_strbuf_get_length(request->response.buffer);
    if (buffer_len) {
        vec[last++] = (struct iovec){
            .iov_base = "data: ",
        ...
        vec[last++] = (struct iovec){
            .iov_base = turboserve_strbuf_get_buffer(request->response.buffer),
            .iov_len = buffer_len,
        };
    }
    ...
    turboserve_writev(request, vec, last);
