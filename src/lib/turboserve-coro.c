...
turboserve_SELF_TEST(sizes_are_same)
{
...
asm(".text\n\t"
    ...
    "mov sp, x10\n\t"
    "br x11\n\t");
#elif defined(turboserve_HAVE_LIBUCONTEXT)
...
{
    struct turboserve_array *array = (struct turboserve_array *)&coro->defer;
...
{
    const struct turboserve_array *array = (struct turboserve_array *)&coro->defer;
...
coro_new(struct coro_switcher *switcher, coro_function_t function, void *data)
{
    ...
    coro = turboserve_aligned_alloc(sizeof(*coro), 64);
    if (UNLIKELY(!coro)) {
    ...
    coro->stack = stack;
#else
    coro = turboserve_aligned_alloc(sizeof(struct coro) + CORO_STACK_SIZE, 64);
                                       ...
                                       struct coro_defer *defer)
{
    ...
    if (defer == coro_defer_array_get_elem(&coro->defer, num_defers - 1)) {
         ...
         * disarmed_defer(). */
        struct turboserve_array *defer_base = (struct turboserve_array *)&coro->defer;
        defer_base->elements--;
    } else {
...
coro_defer(struct coro *coro, defer1_func func, void *data)
{
    ...
    if (UNLIKELY(!defer)) {
        turboserve_status_error("Could not add new deferred function for coro %p",
                          coro);
        return -1;
    }
