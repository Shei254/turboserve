...
void turboserve_job_thread_main_loop(void)
{
    ...
    turboserve_set_thread_name("job");

    if (pthread_mutex_lock(&job_wait_mutex))
        turboserve_status_critical("Could not lock job wait mutex");
...
void turboserve_job_thread_init(void)
{
    ...
    turboserve_status_debug("Initializing low priority job thread");
    ...
    if (pthread_setschedparam(self, SCHED_IDLE, &sched_param) < 0)
        turboserve_status_perror("pthread_setschedparam");
#endif  /* SCHED_IDLE */
}

void turboserve_job_thread_shutdown(void)
{
    turboserve_status_debug("Shutting down job thread");
    ...
    if (LIKELY(!pthread_mutex_lock(&queue_mutex))) {
        ...
        if (r) {
            errno = r;
            turboserve_status_perror("pthread_join");
        }
...
void turboserve_job_add(bool (*cb)(void *data), void *data)
{
    ...
    if (LIKELY(!pthread_mutex_lock(&queue_mutex))) {
        ...
        turboserve_status_warning("Couldn't lock job mutex");
        free(job);
    }
}
