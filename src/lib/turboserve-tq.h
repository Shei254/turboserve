...
struct timeout_queue {
    const struct turboserve *turboserve;
    struct turboserve_connection *conns;
    struct turboserve_connection head;
    struct timeout timeout;
    unsigned int current_time;
    unsigned int move_to_last_bump;
};

void timeout_queue_init(struct timeout_queue *tq, const struct turboserve *l);

void timeout_queue_insert(struct timeout_queue *tq,
                          struct turboserve_connection *new_node);
void timeout_queue_expire(struct timeout_queue *tq, struct turboserve_connection *node);
