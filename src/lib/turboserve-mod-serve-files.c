...
static const struct turboserve_key_value deflate_compression_hdr[] = {
    {"Content-Encoding", "deflate"}, {}
};
static const struct turboserve_key_value gzip_compression_hdr[] = {
    {"Content-Encoding", "gzip"}, {}
};
#if defined(turboserve_HAVE_BROTLI)
static const struct turboserve_key_value br_compression_hdr[] = {
    {"Content-Encoding", "br"}, {}
};
#endif
#if defined(turboserve_HAVE_ZSTD)
static const struct turboserve_key_value zstd_compression_hdr[] = {
...
struct serve_files_priv {
    ...
    struct turboserve_tpl *directory_list_tpl;

    size_t read_ahead;
};

struct cache_funcs {
    enum turboserve_http_status (*serve)(struct turboserve_request *request, void *data);
...
struct mmap_cache_data {
    struct turboserve_value uncompressed;
    struct turboserve_value gzip;
    struct turboserve_value deflated;
#if defined(turboserve_HAVE_BROTLI)
    struct turboserve_value brotli;
#endif
#if defined(turboserve_HAVE_ZSTD)
    struct turboserve_value zstd;
#endif
...
struct dir_list_cache_data {
    struct turboserve_strbuf rendered;
    struct turboserve_value deflated;
#if defined(turboserve_HAVE_BROTLI)
    struct turboserve_value brotli;
#endif
...
static enum turboserve_http_status mmap_serve(struct turboserve_request *request,
                                        void *data);
...
static enum turboserve_http_status sendfile_serve(struct turboserve_request *request,
                                            void *data);
...
static enum turboserve_http_status dirlist_serve(struct turboserve_request *request,
                                           void *data);
...
static enum turboserve_http_status redir_serve(struct turboserve_request *request,
                                         void *data);
...
static const struct turboserve_var_descriptor file_list_desc[] = {
    ...
    TPL_VAR_SEQUENCE(file_list,
                     ...
                     ((const struct turboserve_var_descriptor[]){
...
static int directory_list_generator(struct coro *coro, void *data)
{
    ...
    while ((entry = readdir(dir))) {
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            fl->file_list.type =
                turboserve_determine_mime_type_for_file_name(entry->d_name);
            fl->file_list.slash_if_dir = "";
        } else {
            continue;
        }
...
static void realloc_if_needed(struct turboserve_value *value, size_t bound)
{
    if (bound > value->len) {
...
static void deflate_value(const struct turboserve_value *uncompressed,
                          struct turboserve_value *compressed)
{
...
#if defined(turboserve_HAVE_BROTLI)
static void brotli_value(const struct turboserve_value *uncompressed,
                         struct turboserve_value *brotli,
                         const struct turboserve_value *deflated)
{
...
#if defined(turboserve_HAVE_ZSTD)
static void zstd_value(const struct turboserve_value *uncompressed,
                       struct turboserve_value *zstd,
                       const struct turboserve_value *deflated)
{
...
{
    turboserve_readahead_queue(fd, 0, turboserve_MIN(size, priv->read_ahead));
}
                    ...
                    struct turboserve_value *value)
{
    ...
    *value = (struct turboserve_value){.value = ptr, .len = size};
    return true;

fail:
    *value = (struct turboserve_value){};
    return false;
}
                      ...
                      struct stat *st)
{
    ...
    if (!mmap_fd(priv, file_fd, (size_t)st->st_size, &md->uncompressed))
        return false;
    turboserve_madvise_queue(md->uncompressed.value, md->uncompressed.len);
    ...
    if (LIKELY(priv->flags & SERVE_FILES_SERVE_PRECOMPRESSED)) {
        ...
        md->gzip = (struct turboserve_value){};
    }
    ...
    ce->mime_type =
        turboserve_determine_mime_type_for_file_name(full_path + priv->root_path_len);
                          ...
                          struct stat *st)
{
    ...
    ce->mime_type = turboserve_determine_mime_type_for_file_name(relpath);
...
static const char *dirlist_find_readme(struct turboserve_strbuf *readme,
                                       struct serve_files_priv *priv,
                                       const char *full_path)
{
    ...
    for (size_t i = 0; i < N_ELEMENTS(candidates); i++) {
        ...
        if (turboserve_strbuf_init_from_file(readme, readme_path))
            return turboserve_strbuf_get_buffer(readme);
    }
                         ...
                         struct stat *st __attribute__((unused)))
{
    ...
    struct turboserve_strbuf readme;
    bool ret = false;

    if (!turboserve_strbuf_init(&readme))
        return false;
    if (!turboserve_strbuf_init(&dd->rendered))
    ...
    if (!turboserve_tpl_apply_with_buffer(priv->directory_list_tpl, &dd->rendered,
                                    &vars))
        goto out_free_rendered;

    ce->mime_type = "text/html";

    struct turboserve_value rendered = turboserve_strbuf_to_value(&dd->rendered);
...
out_free_rendered:
    turboserve_strbuf_free(&dd->rendered);
out_free_readme:
    turboserve_strbuf_free(&readme);
    return ret;
}
                                              ...
                                              __attribute__((unused)))
{
    ...
    if (UNLIKELY(turboserve_format_rfc_time(st.st_mtime, fce->last_modified.string) <
                 0)) {
...
{
    struct turboserve_serve_files_settings *settings = args;
    struct serve_files_priv *priv;
    char *canonical_root;
    int root_fd;

    if (!settings->root_path) {
        turboserve_status_error("root_path not specified");
        return NULL;
    }

    canonical_root = get_real_root_path(settings->root_path);
    if (!canonical_root) {
        turboserve_status_perror("Could not obtain real path of \"%s\"",
                           settings->root_path);
        goto out_realpath;
    }
    ...
    if (root_fd < 0) {
        turboserve_status_perror("Could not open directory \"%s\"", canonical_root);
        goto out_open;
    }

    priv = malloc(sizeof(*priv));
    if (!priv) {
        turboserve_status_perror("malloc");
        goto out_malloc;
    }
    ...
    priv->cache = cache_create(create_cache_entry, destroy_cache_entry, priv,
                               settings->cache_for);
    if (!priv->cache) {
        turboserve_status_error("Couldn't create cache");
        goto out_cache_create;
    }

    if (settings->directory_list_template) {
        priv->directory_list_tpl = turboserve_tpl_compile_file(
            settings->directory_list_template, file_list_desc);
    } else {
        priv->directory_list_tpl =
            turboserve_tpl_compile_value_full(servefile_template_value,
                                        file_list_desc,
                                        turboserve_TPL_FLAG_CONST_TEMPLATE);
    }
    if (!priv->directory_list_tpl) {
        turboserve_status_error("Could not compile directory list template");
        goto out_tpl_compile;
    }

    priv->prefix = strdup(prefix);
    if (!priv->prefix) {
        turboserve_status_error("Could not copy prefix");
...
{
    struct turboserve_serve_files_settings settings = {
...
static void serve_files_destroy(void *data)
{
    ...
    if (!priv) {
        turboserve_status_warning("Nothing to shutdown");
        return;
    }

    turboserve_tpl_free(priv->directory_list_tpl);
    cache_destroy(priv->cache);
    close(priv->root_fd);
    free(priv->root_path);
    free(priv->prefix);
...
static ALWAYS_INLINE bool client_has_fresh_content(struct turboserve_request *request,
                                                   time_t mtime)
{
    time_t header;
    int r = turboserve_request_get_if_modified_since(request, &header);

    return LIKELY(!r) ? mtime <= header : false;
}

static size_t prepare_headers(struct turboserve_request *request,
                              enum turboserve_http_status return_status,
                              struct file_cache_entry *fce,
                              size_t size,
                              const struct turboserve_key_value *user_hdr,
                              char header_buf[static DEFAULT_HEADERS_SIZE])
{
    char content_length[INT_TO_STR_BUFFER_SIZE];
    size_t discard;
    struct turboserve_key_value additional_headers[4] = {
        {
            .key = "Last-Modified",
    ...
    return turboserve_prepare_response_header_full(request, return_status, header_buf,
                                             DEFAULT_HEADERS_SIZE,
                                             additional_headers);
}

static enum turboserve_http_status
compute_range(struct turboserve_request *request, off_t *from, off_t *to, off_t size)
{
    off_t f, t;
    int r = turboserve_request_get_range(request, &f, &t);
...
static inline bool accepts_encoding(struct turboserve_request *request,
                                    const enum turboserve_request_flags encoding)
{
    return turboserve_request_get_accept_encoding(request) & encoding;
}

static enum turboserve_http_status sendfile_serve(struct turboserve_request *request,
                                            void *data)
{
    const struct turboserve_key_value *compression_hdr;
    ...
    if (turboserve_request_get_method(request) == REQUEST_METHOD_HEAD) {
        turboserve_send(request, headers, header_len, 0);
    } else {
        turboserve_sendfile(request, fd, from, (size_t)to, headers, header_len);
    }

    return return_status;
}

static enum turboserve_http_status serve_buffer(struct turboserve_request *request,
                                          const char *mime_type,
                                          const void *buffer,
                                          size_t buffer_len,
                                          const struct turboserve_key_value *headers,
                                          enum turboserve_http_status status_code)
{
    request->response.mime_type = mime_type;
    request->response.headers = headers;

    turboserve_strbuf_set_static(request->response.buffer, buffer, buffer_len);

    return status_code;
}

static ALWAYS_INLINE enum turboserve_http_status
serve_value(struct turboserve_request *request,
            const char *mime_type,
...
static ALWAYS_INLINE enum turboserve_http_status
serve_value_ok(struct turboserve_request *request,
               const char *mime_type,
               const struct turboserve_value *value,
               const struct turboserve_key_value *headers)
{
...
static const struct turboserve_value *
mmap_best_data(struct turboserve_request *request,
               struct mmap_cache_data *md,
               const struct turboserve_key_value **header)
{
    const struct turboserve_value *best = &md->uncompressed;
                                        ...
                                        void *data)
{
    ...
    struct mmap_cache_data *md = &fce->mmap_cache_data;
    const struct turboserve_key_value *compression_hdr;
    const struct turboserve_value *to_serve =
    ...
    if (ATOMIC_DEC(md->mincore_call_threshold) == 0) {
        ...
        if (!mincore(to_serve->value, to_serve->len, mincore_vec)) {
            ...
            for (size_t pg = 0; pg < pgs; pg++) {
                ...
                turboserve_madvise_queue(to_serve->value, to_serve->len);
                                           ...
                                           void *data)
{
    struct file_cache_entry *fce = data;
    struct dir_list_cache_data *dd = &fce->dir_list_cache_data;
    const char *icon = turboserve_request_get_query_param(request, "icon");
    ...
    if (!icon) {
        ...
        return serve_buffer(
            request, fce->mime_type, turboserve_strbuf_get_buffer(&dd->rendered),
            turboserve_strbuf_get_length(&dd->rendered), NULL, HTTP_OK);
    }
                                         ...
                                         void *data)
{
    struct file_cache_entry *fce = data;
    struct redir_cache_data *rd = &fce->redir_cache_data;
    struct turboserve_key_value headers[] = {{"Location", rd->redir_to}, {}};

    turboserve_strbuf_set_staticz(request->response.buffer, rd->redir_to);
...
static enum turboserve_http_status
serve_files_handle_request(struct turboserve_request *request,
...
turboserve_REGISTER_MODULE(serve_files, &module);
