...
struct turboserve_redirect_settings {
    char *to;
    enum turboserve_http_status code;
};

turboserve_MODULE_FORWARD_DECL(redirect)

#define REDIRECT_CODE(to_, code_)                                              \
    .module = turboserve_MODULE_REF(redirect),                                       \
    .args = ((struct turboserve_redirect_settings[]) {{                              \
        .to = (to_),                                                           \
        .code = (code_),                                                       \
    }}),                                                                       \
