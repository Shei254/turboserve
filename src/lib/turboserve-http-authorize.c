...
{
    if (LIKELY(str)) {
        turboserve_always_bzero(str, strlen(str));
        free(str);
    }
}
                  ...
                  void *create_contex __attribute__((unused)))
{
    ...
    if (config_last_error(f)) {
        turboserve_status_error("Error on password file \"%s\", line %d: %s", (char *)key,
...
bool turboserve_http_authorize_init(void)
{
...
void turboserve_http_authorize_shutdown(void) { cache_destroy(realm_password_cache); }
...
bool turboserve_http_authorize(struct turboserve_request *request,
                         const char *realm,
                         const char *password_file)
{
    ...
    const char *authorization =
        turboserve_request_get_header(request, "Authorization");
