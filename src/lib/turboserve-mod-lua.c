...
{
    struct turboserve_lua_priv *priv = cache_ctx;
    struct turboserve_lua_state *state = malloc(sizeof(*state));

    if (UNLIKELY(!state))
        return NULL;

    state->L = turboserve_lua_create_state(priv->script_file, priv->script);
    if (LIKELY(state->L))
...
{
    struct turboserve_lua_state *state = (struct turboserve_lua_state *)entry;

    lua_close(state->L);
    free(state);
}

static struct cache *get_or_create_cache(struct turboserve_lua_priv *priv)
{
    struct cache *cache = pthread_getspecific(priv->cache_key);

    if (UNLIKELY(!cache)) {
        turboserve_status_debug("Creating cache for this thread");
        cache =
            cache_create(state_create, state_destroy, priv, priv->cache_period);
        if (UNLIKELY(!cache))
            turboserve_status_error("Could not create cache");
...
static ALWAYS_INLINE struct turboserve_value
get_handle_prefix(struct turboserve_request *request)
{
...
#define GEN_TABLE_ENTRY(upper, lower, mask, constant, probability)             \
    ...
    static const struct turboserve_value method2name[REQUEST_METHOD_MASK] = {
...
#undef ENTRY
    ...
    return method2name[turboserve_request_get_method(request)];
}

static bool get_handler_function(lua_State *L, struct turboserve_request *request)
{
    char handler_name[128];
    struct turboserve_value handle_prefix = get_handle_prefix(request);
...
static enum turboserve_http_status lua_handle_request(struct turboserve_request *request,
                                                struct turboserve_response *response,
                                                void *instance)
{
    struct turboserve_lua_priv *priv = instance;

    struct cache *cache = get_or_create_cache(priv);
    if (UNLIKELY(!cache))
        return HTTP_INTERNAL_ERROR;

    struct turboserve_lua_state *state =
        (struct turboserve_lua_state *)cache_coro_get_and_ref_entry(
        ...
        case 0:
            ...
            if (lua_isnumber(L, -1)) {
                ...
                if (code >= 100 && code <= 999)
                    return (enum turboserve_http_status)code;
            }

            return HTTP_INTERNAL_ERROR;
        default:
            turboserve_status_error("Error from Lua script: %s", lua_tostring(L, -1));
            return HTTP_INTERNAL_ERROR;
        }
    }
}
...
{
    struct turboserve_lua_settings *settings = data;
    struct turboserve_lua_priv *priv;

    priv = calloc(1, sizeof(*priv));
    if (!priv) {
        turboserve_status_error("Could not allocate memory for private Lua struct");
        return NULL;
    }
    ...
    if (!priv->default_type) {
        turboserve_status_perror("strdup");
        goto error;
    }
    ...
    if (settings->script) {
        ...
        turboserve_status_error("No Lua script_file or script provided");
        goto error;
    }

    if (pthread_key_create(&priv->cache_key, NULL)) {
        turboserve_status_perror("pthread_key_create");
        goto error;
    }
...
{
    struct turboserve_lua_settings settings = {
...
turboserve_REGISTER_MODULE(lua, &module);
