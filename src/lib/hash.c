...
#if defined(turboserve_HAVE_BUILTIN_CPU_INIT) && defined(turboserve_HAVE_BUILTIN_IA32_CRC32)
...
turboserve_CONSTRUCTOR(fnv1a_seed, 65535)
{
    uint8_t entropy[128];

    /* The seeds are randomized in order to mitigate the DDoS attack
     * described by Crosby and Wallach in UsenixSec2003.  */
    if (UNLIKELY(turboserve_getentropy(entropy, sizeof(entropy), 0) < 0)) {
        turboserve_status_critical_perror("Could not initialize FNV1a seed");
        __builtin_unreachable();
    }

    fnv1a_64_seed = fnv1a_64(entropy, sizeof(entropy));
    fnv1a_32_seed = fnv1a_32(entropy, sizeof(entropy));
    turboserve_always_bzero(entropy, sizeof(entropy));

#if defined(turboserve_HAVE_BUILTIN_CPU_INIT) && defined(turboserve_HAVE_BUILTIN_IA32_CRC32)
    __builtin_cpu_init();
    if (__builtin_cpu_supports("sse4.2")) {
        turboserve_status_debug("Using CRC32 instructions to calculate hashes");
