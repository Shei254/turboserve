#!/usr/bin/python
# TODO: Use tracy (https://github.com/MerlijnWajer/tracy) to see if turboserve
...
class turboserveTest(unittest.TestCase):
  harness_paths = {
  ...
  def setUp(self, env=None, harness='testrunner'):
    ...
    for spawn_try in range(20):
      self.turboserve = subprocess.Popen([self.harness_paths[harness]], env=env,
                                   stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)

      if self.turboserve.poll() is not None:
    ...
    raise Exception('Timeout waiting for turboserve')

  def tearDown(self):
    try:
      requests.get('http://127.0.0.1:8080/quit-turboserve')
    except requests.exceptions.ConnectionError:
      # Requesting /quit-turboserve will make testrunner exit(0), closing the
      # connection without sending a response, raising this exception.
      # That's expected here.
      return
    finally:
      with self.turboserve as l:
...
class TestPost(turboserveTest):
  ...
  # These two tests are supposed to fail, with turboserve aborting the connection.
...
class TestFileServing(turboserveTest):
  ...
  def test_has_turboserve_server_header(self):
    r = requests.get('http://127.0.0.1:8080/100.html')
    self.assertTrue('server' in r.headers)
    self.assertEqual(r.headers['server'], 'turboserve/testrunner')
...
class TestRewrite(turboserveTest):
...
class SocketTest(turboserveTest):
  class WrappedSock:
...
class TestChunkedEncoding(turboserveTest):
...
class TestAuthentication(turboserveTest):
  class TempHtpasswd:
...
class TestQueryString(turboserveTest):
...
class TestHelloWorld(turboserveTest):
...
class TestCache(turboserveTest):
  def setUp(self):
    self._cache_for = 3
  ...
  def mmaps(self, f):
    with open('/proc/%d/maps' % self.turboserve.pid) as map_file:
      f = f + '\n'
...
class TestHeaderParsing(SocketTest):
  def test_check_for_crlf(self):
    # https://github.com/lpereira/turboserve/issues/368
...
class TestArtificialResponse(turboserveTest):
  def test_brew_coffee(self):
...
class TestSleep(turboserveTest):
  def test_sleep(self):
    now = time.time()
...
class TestRequest(turboserveTest):
