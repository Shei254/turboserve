...
""")
  ...
  print("""DEFINE_RING_BUFFER_TYPE(uint8_ring_buffer, uint8_t, 64)

struct turboserve_h2_huffman_decoder {
    struct bit_reader bit_reader;
    struct uint8_ring_buffer buffer;
};

void turboserve_h2_huffman_init(struct turboserve_h2_huffman_decoder *huff,
                          const uint8_t *input,
                          size_t input_len)
{
...
ssize_t turboserve_h2_huffman_next(struct turboserve_h2_huffman_decoder *huff)
{
...
bool turboserve_h2_huffman_decode_for_fuzzing(const uint8_t *input, size_t input_len)
{
    struct turboserve_h2_huffman_decoder decoder;

    turboserve_h2_huffman_init(&decoder, input, input_len);

    while (true) {
        ssize_t n_decoded = turboserve_h2_huffman_next(&decoder);
...
bool test_decoder(unsigned char input[], size_t input_size, const char expected[], size_t expected_size)
{
    struct turboserve_h2_huffman_decoder decoder;
    const char *expected_ptr = expected;

    turboserve_h2_huffman_init(&decoder, input, input_size);

    while (true) {
        ssize_t n_decoded = turboserve_h2_huffman_next(&decoder);
        if (n_decoded < 0)
            return false;
        if (n_decoded == 0 && expected_size == 0)
            return true;
        if (n_decoded == 0 && expected_size != 0)
            return false;
...
{
    /* "litespeed" */
    unsigned char litespeed_huff[] = {0xce, 0x64, 0x97, 0x75, 0x65, 0x2c, 0x9f};
    if (!test_decoder(litespeed_huff, sizeof(litespeed_huff), "LiteSpeed",
                      sizeof("LiteSpeed") - 1)) {
        return 1;
    }
    ...
    unsigned char x_fb_debug_decoded[] =
        "mEO7bfwFStBMwJWfW4pmg2XL25AswjrVlfcfYbxkcS2ssduZmiKoipMH9XwoTGkb+"
        "Qnq9bcjwWbwDQzsea/vMQ==";
    if (!test_decoder(x_fb_debug, sizeof(x_fb_debug), x_fb_debug_decoded,
                      sizeof(x_fb_debug_decoded) - 1)) {
        return 1;
    }
