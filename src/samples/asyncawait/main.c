...
turboserve_HANDLER_ROUTE(asyncawait, "/")
{
    int fd;
    ...
    while (true) {
        char buffer[128];
        ssize_t r = turboserve_recv_fd(request, fd, buffer, sizeof(buffer), 0);

        turboserve_strbuf_set_static(response->buffer, buffer, (size_t)r);
...
int main(void) { return turboserve_main(); }
