...
static const struct turboserve_var_descriptor fortune_desc[] = {
...
static struct turboserve_tpl *fortune_tpl;
...
static struct db *get_db(void)
{
    ...
    if (!database) {
        ...
        if (!database)
            turboserve_status_critical("Could not connect to the database");
    }
...
json_response_obj(struct turboserve_response *response,
...
static enum turboserve_http_status
json_response_arr(struct turboserve_response *response,
                  const struct json_obj_descr *descr,
                  const void *data)
{
...
static long get_number_of_queries(struct turboserve_request *request)
{
    const char *queries_str = turboserve_request_get_query_param(request, "queries");
    return LIKELY(queries_str)
               ? turboserve_MIN(500, turboserve_MAX(1, parse_long(queries_str, -1)))
               : 1;
}

turboserve_HANDLER(queries)
{
    enum turboserve_http_status ret = HTTP_INTERNAL_ERROR;
    ...
    turboserve_strbuf_grow_to(response->buffer, (size_t)(32l * queries));
...
turboserve_HANDLER(cached_queries)
{
    long queries = get_number_of_queries(request);
    ...
    for (long i = 0; i < queries; i++) {
        ...
        int key = (int)turboserve_random_uint64() % 10000;
        int error;
...
turboserve_HANDLER(plaintext)
{
    turboserve_strbuf_set_static(response->buffer, hello_world,
...
static int fortune_list_generator(struct coro *coro, void *data)
{
    ...
    struct Fortune *iter;
    turboserve_ARRAY_FOREACH (&fortunes, iter) {
...
turboserve_HANDLER(fortunes)
{
    struct Fortune fortune;

    turboserve_strbuf_grow_to(response->buffer, 1500);

    if (UNLIKELY(!turboserve_tpl_apply_with_buffer(fortune_tpl, response->buffer,
                                             &fortune)))
        return HTTP_INTERNAL_ERROR;
...
int main(void)
{
    ...
    if (getenv("USE_MYSQL")) {
        ...
        if (!db_connection_params.mysql.user)
            turboserve_status_critical("No MySQL user provided");
        if (!db_connection_params.mysql.password)
            turboserve_status_critical("No MySQL password provided");
        if (!db_connection_params.mysql.hostname)
            turboserve_status_critical("No MySQL hostname provided");
        if (!db_connection_params.mysql.database)
            turboserve_status_critical("No MySQL database provided");
    } else {
    ...
    fortune_tpl = turboserve_tpl_compile_string_full(
        fortunes_template_str, fortune_desc, turboserve_TPL_FLAG_CONST_TEMPLATE);
    if (!fortune_tpl)
        turboserve_status_critical("Could not compile fortune templates");
    ...
    if (!cached_queries_cache)
        turboserve_status_critical("Could not create cached queries cache");
    ...
    turboserve_tpl_free(fortune_tpl);
