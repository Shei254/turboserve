...
static void write_num(struct turboserve_strbuf *buf, int16_t num)
{
    turboserve_strbuf_append_char(buf, num & 0xff);
    turboserve_strbuf_append_char(buf, num >> 8);
}

ge_GIF *ge_new_gif(struct turboserve_strbuf *buf,
                   uint16_t width,
                   uint16_t height,
                   uint8_t *palette,
                   int depth,
                   int loop)
{
    ...
    turboserve_strbuf_append_str(buf, "GIF89a", 6);
    write_num(buf, width);
    write_num(buf, height);
    turboserve_strbuf_append_str(buf, (const char[]){0xF0 | (depth - 1), 0x00, 0x00},
                           3);

    if (palette) {
        turboserve_strbuf_append_str(buf, palette, 3ull << depth);
    } else if (depth <= 4) {
        turboserve_strbuf_append_str(buf, vga, 3ull << depth);
    } else {
        turboserve_strbuf_append_str(buf, vga, sizeof(vga));

        i = 0x10;
        for (r = 0; r < 6; r++) {
            for (g = 0; g < 6; g++) {
                for (b = 0; b < 6; b++) {
                    turboserve_strbuf_append_str(
        ...
        for (i = 1; i <= 24; i++) {
            v = i * 0xFF / 25;
            turboserve_strbuf_append_str(buf, (const char[]){v, v, v}, 3);
        }
    }
...
static void put_loop(ge_GIF *gif, uint16_t loop)
{
    turboserve_strbuf_append_str(gif->buf, (const char[]){'!', 0xFF, 0x0B}, 3);
    turboserve_strbuf_append_str(gif->buf, "NETSCAPE2.0", 11);
    turboserve_strbuf_append_str(gif->buf, (const char[]){0x03, 0x01}, 2);
    write_num(gif->buf, loop);
    turboserve_strbuf_append_char(gif->buf, '\0');
}
...
static void put_key(ge_GIF *gif, uint16_t key, int key_size)
{
    ...
    while (bits_to_write >= 8) {
        ...
        if (byte_offset == 0xFF) {
            turboserve_strbuf_append_char(gif->buf, 0xff);
            turboserve_strbuf_append_str(gif->buf, gif->buffer, 0xff);
...
static void end_key(ge_GIF *gif)
{
    ...
    turboserve_strbuf_append_char(gif->buf, byte_offset);
    turboserve_strbuf_append_str(gif->buf, gif->buffer, byte_offset);
    turboserve_strbuf_append_char(gif->buf, '\0');
...
put_image(ge_GIF *gif, uint16_t w, uint16_t h, uint16_t x, uint16_t y)
{
    ...
    turboserve_strbuf_append_char(gif->buf, ',');
    write_num(gif->buf, x);
    write_num(gif->buf, y);
    write_num(gif->buf, w);
    write_num(gif->buf, h);
    turboserve_strbuf_append_str(gif->buf, (const char[]){0x00, gif->depth}, 2);
...
{
    turboserve_strbuf_append_str(gif->buf, (const char[]){'!', 0xF9, 0x04, 0x04}, 4);
    write_num(gif->buf, d);
    turboserve_strbuf_append_str(gif->buf, "\0\0", 2);
}
...
struct turboserve_strbuf *ge_close_gif(ge_GIF *gif)
{
    struct turboserve_strbuf *buf = gif->buf;
