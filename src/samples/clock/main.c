...
#include "turboserve-mod-redirect.h"
#include "gifenc.h"
...
static const struct turboserve_key_value seriously_do_not_cache[] = {
...
turboserve_HANDLER(clock)
{
    ...
    response->headers = (struct turboserve_key_value *)seriously_do_not_cache;
    ...
    for (int frame = 0; frame < 3600 * 2; frame++) {
        ...
        turboserve_request_sleep(request, 500);
    }
...
turboserve_HANDLER(dali)
{
    ...
    for (uint32_t frame = 0; frame < one_hour; frame++) {
        ...
        ge_add_frame(gif, 0);
        turboserve_response_send_chunk(request);
        turboserve_request_sleep(request, xdaliclock_get_frame_time(xdc));
    }

    return HTTP_OK;
}

turboserve_HANDLER(blocks)
{
    ...
    while (total_waited <= 3600000) {
        ...
        ge_add_frame(gif, 0);
        turboserve_response_send_chunk(request);
        turboserve_request_sleep(request, timeout);
    }

    return HTTP_OK;
}

turboserve_HANDLER(pong)
{
...
static const struct turboserve_var_descriptor index_desc[] = {
...
static struct turboserve_tpl *index_tpl;

turboserve_CONSTRUCTOR(initialize_template, 0)
{
    static const char index[] =
        ...
        "   width: 300px;\n"
        "}\n"
        "#styles a, #styles a:visited, #turboserve a, #turboserve a:visited { color: #666; }\n"
        "#turboserve {\n"
        "   color: #555;\n"
        "   top: calc(100% - 40px);\n"
        "   position: absolute;\n"
        ...
        "<body>\n"
        "  <div id=\"turboserve\">\n"
        "    Powered by the <a href=\"https://turboserve.ws\">turboserve</a> web server.\n"
        "  </div>\n"
    ...
    index_tpl = turboserve_tpl_compile_string_full(index, index_desc,
                                             turboserve_TPL_FLAG_CONST_TEMPLATE);
    if (!index_tpl)
        turboserve_status_critical("Could not compile template");
}

turboserve_HANDLER(templated_index)
{
    if (turboserve_tpl_apply_with_buffer(index_tpl, response->buffer, data)) {
...
int main(void)
{
    struct index sample_clock = {
        .title = "turboserve Sample Clock",
        .variant = "clock",
        .width = 200,
    };
    struct index dali_clock = {
        .title = "turboserve Dali Clock",
        .variant = "dali",
        .width = 320,
    };
    struct index blocks_clock = {
        .title = "turboserve Blocks Clock",
        .variant = "blocks",
        .width = 320,
    };
    struct index pong_clock = {
        .title = "turboserve Pong Clock",
        .variant = "pong",
        .width = 320,
    };
    const struct turboserve_url_map default_map[] = {
        {
            .prefix = "/clock.gif",
            .handler = turboserve_HANDLER_REF(clock),
        },
        {
            .prefix = "/dali.gif",
            .handler = turboserve_HANDLER_REF(dali),
        },
        {
            .prefix = "/blocks.gif",
            .handler = turboserve_HANDLER_REF(blocks),
        },
        {
            .prefix = "/pong.gif",
            .handler = turboserve_HANDLER_REF(pong),
        },
        {
            .prefix = "/clock",
            .handler = turboserve_HANDLER_REF(templated_index),
            .data = &sample_clock,
        },
        {
            .prefix = "/dali",
