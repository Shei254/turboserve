...
{
    const struct turboserve_value *body = create_ctx;
    size_t alloc_size;
...
static enum turboserve_http_status post_paste(struct turboserve_request *request,
                                        struct turboserve_response *response)
{
    ...
    for (int try = 0; try < 10; try++) {
        ...
        do {
            key = (void *)(uintptr_t)turboserve_random_uint64();
        } while (!key);
        ...
        if (paste) {
            const char *host_hdr = turboserve_request_get_host(request);

            if (!host_hdr)
                return HTTP_BAD_REQUEST;

            response->mime_type = "text/plain";
            turboserve_strbuf_printf(response->buffer, "https://%s/p/%zu\n\n",
...
static enum turboserve_http_status doc(struct turboserve_request *request,
                                 struct turboserve_response *response)
{
    const char *host_hdr = turboserve_request_get_host(request);
...
turboserve_HANDLER_ROUTE(view_paste, "/p/")
{
    char *dot = memrchr(request->url.value, '.', request->url.len);
    const char *mime_type;

    if (dot) {
        mime_type = turboserve_determine_mime_type_for_file_name(dot);
        *dot = '\0';
    } else {
    ...
    turboserve_strbuf_set_static(response->buffer, paste->value, paste->len);
...
int main(void)
{
    ...
    if (!pastes)
        turboserve_status_critical("Could not create paste cache");
