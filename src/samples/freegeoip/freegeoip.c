...
static const struct turboserve_var_descriptor template_descriptor[] = {
...
static const char csv_template_str[] = "\"{{ip}}\","
                                       "\"{{country.code}}\","
                                       "\"{{country.name}}\","
                                       "\"{{region.code}}\","
                                       "\"{{region.name}}\","
...
static bool net_contains_ip(const struct ip_net *net, in_addr_t ip)
{
...
static bool is_reserved_ip(in_addr_t ip)
{
    size_t i;
...
static void destroy_ipinfo(struct cache_entry *entry,
...
static ALWAYS_INLINE char *text_column_helper(sqlite3_stmt *stmt, int ind)
{
                                         ...
                                         void *create_ctx __attribute__((unused)))
{
    ...
    if (sqlite3_prepare(db, ip_to_city_query, sizeof(ip_to_city_query) - 1,
                        &stmt, NULL) != SQLITE_OK)
        goto end_no_finalize;
...
static void destroy_query_limit(struct cache_entry *entry,
                                void *context __attribute__((unused)))
{
    free(entry);
}
...
static struct ip_info *internal_query(struct turboserve_request *request,
...
turboserve_HANDLER(templated_output)
{
    ...
    ip_address = turboserve_request_get_remote_address(request, ip_address_buf);
    ...
    const char *callback = turboserve_request_get_query_param(request, "callback");
    struct ip_info info_with_callback = *info;
    info_with_callback.callback = callback;

    if (!turboserve_tpl_apply_with_buffer(tm->tpl, response->buffer,
                                    &info_with_callback)) {
        return HTTP_INTERNAL_ERROR;
    }
...
static struct template_mime compile_template(const char *template,
                                             const char *mime_type)
{
    struct turboserve_tpl *tpl = turboserve_tpl_compile_string_full(
        template, template_descriptor, turboserve_TPL_FLAG_CONST_TEMPLATE);

    if (!tpl) {
        turboserve_status_critical("Could not compile template for mime-type %s",
                             mime_type);
    }
...
int main(void)
{
    ...
    if (result != SQLITE_OK)
        turboserve_status_critical("Could not open database: %s", sqlite3_errmsg(db));
...
#if QUERIES_PER_HOUR != 0
    turboserve_status_info("Limiting to %d queries per hour per client",
                     QUERIES_PER_HOUR);
    query_limit =
        cache_create(create_query_limit, destroy_query_limit, NULL, 3600);
#else
    turboserve_status_info("Rate-limiting disabled");
#endif

    const struct turboserve_url_map default_map[] = {
        {.prefix = "/json/",
         .handler = turboserve_HANDLER_REF(templated_output),
         .data = &json_tpl},
        {.prefix = "/xml/",
         .handler = turboserve_HANDLER_REF(templated_output),
         .data = &xml_tpl},
    ...
    turboserve_tpl_free(json_tpl.tpl);
    turboserve_tpl_free(xml_tpl.tpl);
    turboserve_tpl_free(csv_tpl.tpl);
#if QUERIES_PER_HOUR != 0
