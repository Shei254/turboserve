...
dump(struct config *config, int indent_level)
{
    ...
    if (indent_level > 64) {
        turboserve_status_critical("Indent level %d above limit, aborting",
                             indent_level);
        return;
    }
    ...
    while ((line = config_read_line(config))) {
        ...
        case CONFIG_LINE_TYPE_SECTION_END:
            if (indent_level == 0)
                turboserve_status_critical("Section ended before it started");
            return;
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        turboserve_status_critical("Usage: %s /path/to/config/file.conf", argv[0]);
        return 1;
    }

    config = config_open(argv[1]);
    if (!config) {
        turboserve_status_critical_perror("Could not open configuration file %s",
                                    argv[1]);
        return 1;
    }
    ...
    if (config_last_error(config)) {
        turboserve_status_critical("Error while reading configuration file (line %d): %s\n",
