...
static int bin2hex_mmap(const char *path, const char *identifier)
{
    ...
    printf("static const struct turboserve_value %s_value = {.value = (char *)%s, .len = "
           "sizeof(%s)};\n",
...
static int bin2hex_incbin(const char *path, const char *identifier)
{
    ...
    printf("        \"%s_end:\\n\"\n", identifier);
    printf("        \".previous\\n\");\n");
    printf("static struct turboserve_value %s_value;\n", identifier);
...
int main(int argc, char *argv[])
{
    ...
    printf("#pragma once\n\n");
    printf("#include \"turboserve-private.h\"\n");
    ...
    printf("#if defined(__GNUC__) || defined(__clang__)\n");
    printf("turboserve_CONSTRUCTOR(bin2hex_%016lx, 0)\n", (uintptr_t)argv);
    printf("{\n");
    for (arg = 1; arg < argc; arg += 2) {
        const char *identifier = argv[arg + 1];

        printf("    %s_value = (struct turboserve_value) {.value = (char *)%s_start, "
