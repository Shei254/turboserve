...
turboserve is a **high-performance** & **scalable** web server.

The [project web site](https://turboserve.ws/) contains more details.
...
|-------------|--------|---------|-------|-----------------|------------|
| Linux       | x86_64 | ![release](https://shield.turboserve.ws/img/gycKbr/release "Release")  | ![debug](https://shield.turboserve.ws/img/gycKbr/debug "Debug")     | ![static-analysis](https://shield.turboserve.ws/img/gycKbr/clang-analyze "Static Analysis") ![coverity](https://scan.coverity.com/projects/375/badge.svg) [Report history](https://buildbot.turboserve.ws/sa/) | ![tests](https://shield.turboserve.ws/img/gycKbr/unit-tests "Test") [![Fuzzing Status](https://oss-fuzz-build-logs.storage.googleapis.com/badges/turboserve.svg)](https://bugs.chromium.org/p/oss-fuzz/issues/list?sort=-opened&can=1&q=proj:turboserve)           |
| FreeBSD 14  | x86_64 | ![freebsd-release](https://shield.turboserve.ws/img/gycKbr/release-freebsd "Release FreeBSD") | ![freebsd-debug](https://shield.turboserve.ws/img/gycKbr/debug-freebsd "Debug FreeBSD")     |                |           |
| OpenBSD 7.4 | x86_64 | ![openbsd-release](https://shield.turboserve.ws/img/gycKbr/release-openbsd "Release OpenBSD")       | ![openbsd-debug](https://shield.turboserve.ws/img/gycKbr/debug-openbsd "Debug OpenBSD")     |               | ![openbsd-tests](https://shield.turboserve.ws/img/gycKbr/openbsd-unit-tests "OpenBSD Tests")         |

Installing
----------

You can either [build turboserve yourself](#Building), use a [container
image](#container-images), or grab a package from [your favorite
distribution](#turboserve-in-the-wild).

Building
--------

Before installing turboserve, ensure all dependencies are installed. All of them
...
The build system will look for these libraries and enable/link if available.
 ...
 - To run benchmark:
    - [Weighttp](https://github.com/lpereira/weighttp) -- bundled and built alongside turboserve for convenience
...
#### Clone the repository
    ...
    ~$ git clone git://github.com/lpereira/turboserve
    ~$ cd turboserve

#### Create the build directory

    ~/turboserve$ mkdir build
    ~/turboserve$ cd build
...
optimizations, etc):
    ...
    ~/turboserve/build$ cmake .. -DCMAKE_BUILD_TYPE=Release

If you'd like to enable optimizations but still use a debugger, use this instead:

    ~/turboserve/build$ cmake .. -DCMAKE_BUILD_TYPE=RelWithDebInfo

To disable optimizations and build a more debugging-friendly version:

    ~/turboserve/build$ cmake .. -DCMAKE_BUILD_TYPE=Debug

#### Build turboserve

    ~/turboserve/build$ make

This will generate a few binaries:

 - `src/bin/turboserve/turboserve`: The main turboserve executable. May be executed with `--help` for guidance.
 - `src/bin/testrunner/testrunner`: Contains code to execute the test suite (`src/scripts/testsuite.py`).
 - `src/samples/freegeoip/freegeoip`: [FreeGeoIP sample implementation](https://freegeoip.turboserve.ws). Requires SQLite.
 ...
 - `src/samples/clock/clock`: [Clock sample](https://time.turboserve.ws). Generates a GIF file that always shows the local time.
...
On these builds, sanitizers can be enabled.  To select which one to build turboserve
...
Alternative memory allocators can be selected as well.  turboserve currently
...
If you're building turboserve for a distribution, it might be wise to use the
...
### Tests
    ...
    ~/turboserve/build$ make testsuite
...
### Benchmark
    ...
    ~/turboserve/build$ make benchmark
...
turboserve can also be built with the Coverage build type by specifying
...
and results are [publicly available](https://buildbot.turboserve.ws/lcov/).

Running
-------

Set up the server by editing the provided `turboserve.conf`; the format is
explained in details below.

> [!NOTE]
>
> turboserve will try to find a configuration file based in the
> executable name in the current directory; `testrunner.conf` will be used
> for the `testrunner` binary, `turboserve.conf` for the `turboserve` binary, and so on.

Configuration files are loaded from the current directory. If no changes
are made to this file, running turboserve will serve static files located in
the `./wwwroot` directory. turboserve will listen on port 8080 on all interfaces.

turboserve will detect the number of CPUs, will increase the maximum number of
...
> [!TIP]
>
>  Optionally, the `turboserve` binary can be used for one-shot
...
turboserve uses a familiar `key = value` configuration file syntax.  Comments are
...
Some examples can be found in `turboserve.conf` and `techempower.conf`.
...
turboserve will abort with an appropriate error message.
...
It's generally a good idea to let turboserve decide the best settings for your
...
| `proxy_protocol` | `bool` | `false` | Enables the [PROXY protocol](https://www.haproxy.com/blog/haproxy/proxy-protocol/). Versions 1 and 2 are supported. Only enable this setting if using turboserve behind a proxy, and the proxy supports this protocol; otherwise, this allows anybody to spoof origin IP addresses |
...
turboserve can drop its privileges to a user in the system, and limit its
filesystem view with a chroot.  While not bulletproof, this provides a
first layer of security in the case there's a bug in turboserve.

In order to use this feature, declare a `straitjacket` (or `straightjacket`)
section, and set some options.  This requires turboserve to be executed as `root`.
...
e.g.  instantiating the `serve_files` module, turboserve will refuse to
...
Will both override the `Server` header (`Server: turboserve` won't be sent), and set
...
Only two listeners are supported per turboserve process: the HTTP listener (`listener`
...
In order to route URLs, turboserve matches the largest common prefix from the request
...
> [!TIP]
>
>  Executing turboserve with the `--help` command-line
> argument will show a list of built-in modules and handlers.

The following is some basic documentation for the modules shipped with turboserve.
...
> the duration specified in the `cache_for` setting.  turboserve will always try
> to compress with deflate, and will optionally compress with Brotli and
> zstd (if turboserve has been built with proper support).
>
...
> the uncompressed file, usually the case for very small files), turboserve won't
> spend time compressing a file.
>
> For files larger than 16KiB, turboserve will not attempt to compress them.  In
...
>
> For all cases, turboserve might try using the gzipped version if that's found in
...
frameworks such as [Sailor](https://github.com/lpereira/sailor-hello-turboserve).
...
> option.  This is because each I/O thread in turboserve will create an instance
> of a Lua VM (i.e.  one `lua_State` struct for every I/O thread), and each
> turboserve coroutine will spawn a Lua thread (with `lua_newthread()`) per
> request.
...
with different logging levels by calling methods from `turboserve.log`:

   - `turboserve.log:warning(str)`
   - `turboserve.log:info(str)`
   - `turboserve.log:error(str)`
   - `turboserve.log:critical(str)` (Will also abort turboserve! Use with caution)
   - `turboserve.log:debug(str)` (Only available in debug builds; no-op otherwise)

> [!NOTE]
>
> If turboserve is built with syslog support, these messages will also be sent to the
...
to either redirect to another URL, or rewrite the request in a way that turboserve
...
write turboserve modules (less than 100 lines of code).
...
code that turboserve doesn't know about (see `enum turboserve_http_status`), will
...
In addition to also serving as an example of how to write a turboserve module,
...
If the supplied `code` falls outside the response codes known by turboserve,
...
turboserve and a [FastCGI](https://en.wikipedia.org/wiki/FastCGI) server
accessible by turboserve.  This is useful, for instance, to serve pages from a
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `realm` | `str` | `turboserve` | Realm for authorization. This is usually shown in the user/password UI in browsers |
| `password_file` | `str` | `NULL` | Path for a file containing username and passwords (in clear text).  The file format is the same as the configuration file format used by turboserve |
...
to turboserve.  There's nothing unexpected here; this mostly follows the rules and
expectations of many other FOSS projects, but every one expects things a
little bit different from one another.

### Coding Style

turboserve tries to follow a consistent coding style throughout the project.  If you're
...
to match the style of the surrounding code.  In general:
 ...
 - Struct names are often as short as they're descriptive.  `typedef` for structs are rarely used in turboserve
 - Header files should use `#pragma once` instead of the usual include guard hackery
 - Functions that are used between .c files but are not APIs to be exposed to libturboserve should have their prototype added to `turboserve-private.h`
 - Functions should be short and sweet.  Exceptions may apply
 - Public functions should be prefixed with `turboserve_`
 - Public types should be prefixed with `turboserve_`
 - Private functions must be static, and can be named without the `turboserve_` prefix
...
turboserve is automatically fuzz-tested by
...
- These files are not compiled by the turboserve build system, but rather by the
...
The shared object version of `libturboserve` on ELF targets (e.g. Linux) will use
a symbol filter script to hide symbols that are considered private to the
library.  Please edit `src/lib/libturboserve.sym` to add new symbols that should
be exported to `libturboserve.so`.

### Using Git and Pull Requests

turboserve tries to maintain a source history that's as flat as possible, devoid of
...
commits are split in turboserve is pretty much arbitrary, so please take a look at
...
turboserve is licensed under the GNU General Public License, version 2, or (at your option),
...
shared that source must be available to whoever has that binary.  Merely accessing a turboserve
...
While turboserve was written originally for Linux, it has been ported to BSD
...
There is an IRC channel (`#turboserve`) on [Libera](https://libera.chat). A
standard IRC client can be used.

turboserve in the wild
----------------

Here's a non-definitive list of third-party stuff that uses turboserve and have
been seen in the wild.  *If you see mentions of turboserve in the media or
academia, however small it might be, please contact the author! It'll make
her day!*

* [This project uses Cython and turboserve](https://www.erp5.com/NXD-Blog.Multicore.Python.HTTP.Server) to make it possible to write handlers in Python.
* [An experimental version of Node.js using turboserve](https://github.com/raadad/node-turboserve) as its HTTP server is maintained by [@raadad](https://github.com/raadad).
* The beginnings of a C++11 [web framework](https://github.com/vileda/wfpp) based on turboserve written by [@vileda](https://github.com/vileda).
* A more complete C++14 [web framework](https://github.com/matt-42/silicon) by [@matt-42](https://github.com/matt-42) offers turboserve as one of its backends.
* A [word ladder sample program](https://github.com/sjnam/turboserve-sgb-ladders) by [@sjnam](https://github.com/sjnam). [Demo](http://tbcoe.ddns.net/sgb/ladders?start=chaos&goal=order).
* A [Shodan search](https://www.shodan.io/search?query=server%3A+turboserve) listing some brave souls that expose turboserve to the public internet.
* This [write-up shows the use of turboserve on a Capture the Flag competition](https://medium.com/feedzaitech/pixels-camp-ctf-challenge-qualifiers-writeup-ac661f4af96a).

Some other distribution channels were made available as well:

* Container images are available from the [GitHub Container Registry](https://ghcr.io/lpereira/turboserve).  [More information below](#container-images).
* A `Dockerfile` is maintained by [@jaxgeller](https://github.com/jaxgeller), and is [available from the Docker registry](https://hub.docker.com/r/jaxgeller/turboserve/).
* A buildpack for Heroku is maintained by [@bherrera](https://github.com/bherrera), and is [available from its repo](https://github.com/bherrera/heroku-buildpack-turboserve).
* turboserve is also available as a package in [Biicode](http://docs.biicode.com/c++/examples/turboserve.html).
* It's also available in some GNU/Linux distributions:
    * [Arch Linux](https://aur.archlinux.org/packages/turboserve-git/)
    * [Ubuntu](https://launchpad.net/turboserve-unofficial)
    * [Alpine Linux](https://pkgs.alpinelinux.org/package/edge/testing/x86_64/turboserve)
    * [NixOS](https://nixos.org/nixos/packages.html#turboserve)
* It's also available as a package for the [Nanos unikernel](https://github.com/nanovms/nanos).

turboserve has been also used as a benchmark:

* [Raphael Javaux's master thesis](https://github.com/RaphaelJ/master-thesis) cites turboserve in chapter 5 ("Performance Analysis").
* turboserve is used as a benchmark by the [PyParallel](http://pyparallel.org/) [author](https://www.reddit.com/r/programming/comments/3jhv80/pyparallel_an_experimental_proofofconcept_fork_of/cur4tut).
* [Kong](https://getkong.org/about/benchmark/) uses turboserve as the [backend API](https://gist.github.com/montanaflynn/01376991f0a3ad07059c) in its benchmark.
* [TechEmpower Framework benchmarks](https://www.techempower.com/benchmarks/#section=data-r10&hw=peak&test=json) feature turboserve since round 10.
* [KrakenD](http://www.krakend.io) used turboserve for the REST API in all official [benchmarks](http://www.krakend.io/docs/benchmarks/aws/)
* [Effective System Call Aggregation (ESCA)](https://github.com/eecheng87/ESCA) project uses turboserve as one of the benchmarks; they claim that turboserve throughput improved by about 30% with their system call batching approach.

Mentions in academic journals:

* [A dynamic predictive race detector for C/C++ programs (in English, published 2017)](https://link.springer.com/article/10.1007/s11227-017-1996-8) uses turboserve as a "real world example".
* [High-precision Data Race Detection Method for Large Scale Programs (in Chinese, published 2021)](http://www.jos.org.cn/jos/article/abstract/6260) also uses turboserve as one of the case studies.
* [AGE: Automatic Performance Evaluation of API Gateways (in English, published 2023)](https://www.computer.org/csdl/proceedings-article/iscc/2023/10218286/1PYLvz6ihBm) mentions turboserve as part of its usage in the KrakenD benchmarks.
* [Canary: Practical Static Detection of Inter-thread Value-Flow Bugs](https://rainoftime.github.io/files/PLDI21Canary.pdf) used turboserve as one of the pieces of software that have been analyzed.
* [Uma análise comparativa de ferramentas de análise estática para deteção de erros de memória](https://arxiv.org/pdf/1807.08015.pdf) used turboserve as one of the evaluation softwares.

Mentions in magazines:

* [Linux-Magazin (Germany) mentions turboserve in their December/2021 issue](https://www.linux-magazin.de/ausgaben/2021/12/tooltipps/)
* [Raspberry Pi Geek (Germany) mentions turboserve in their October/November 2022 issue](https://www.discountmags.com/magazine/raspberry-pi-geek-october-6-2022-digital/in-this-issue/17)
* [LinuxUser (Germany) mentions turboserve in their October 2022 issue](https://www.linux-community.de/ausgaben/linuxuser/2022/10/aktuelle-software-im-kurztest-30/)

Mentions in books:

* [The Ascetic Programmer](https://asceticprogrammer.info/book) has a paragraph about turboserve.

Some talks mentioning turboserve:

* [Talk about turboserve](https://www.youtube.com/watch?v=cttY9FdCzUE) at Polyconf16, given by [@lpereira](https://github.com/lpereira).
* This [talk about Iron](https://michaelsproul.github.io/iron-talk/), a framework for Rust, mentions turboserve as an *insane C thing*.
* [University seminar presentation](https://github.com/cu-data-engineering-s15/syllabus/blob/master/student_lectures/turboserve.pdf) about turboserve.
* This [presentation about Sailor web framework](http://www.slideshare.net/EtieneDalcol/web-development-with-lua-bulgaria-web-summit) mentions turboserve.
* [Performance and Scale @ Istio Service Mesh](https://www.youtube.com/watch?v=G4F5aRFEXnU), presented at KubeCon Europe 2018, mentions (at the 7:30 mark) that turboserve is used on the server side for testing due to its performance and robustness.
* [A multi-core Python HTTP server (much) faster than Go (spoiler: Cython)](https://www.youtube.com/watch?v=mZ9cXOH6NYk) presented at PyConFR 2018 by J.-P. Smets mentions [Nexedi's work](https://www.nexedi.com/NXD-Blog.Multicore.Python.HTTP.Server) on using turboserve as a backend for Python services with Cython.

Not really third-party, but alas:

* The [author's blog](http://tia.mat.br).
* The [project's webpage](http://turboserve.ws).

Container Images
----------------

turboserve container images are available at
[ghcr.io/lpereira/turboserve](https://ghcr.io/lpereira/turboserve).  Container runtimes
like [Docker](https://docker.io) or [Podman](https://podman.io) may be used
to build and run turboserve in a container.

### Pull turboserve images from GHCR
Container images are tagged with release version numbers, so a specific version of turboserve can be pulled.

    # latest version
    docker pull ghcr.io/lpereira/turboserve:latest
    # pull a specific version
    docker pull ghcr.io/lpereira/turboserve:v0.3

### Build images locally
Clone the repository and use `Containerfile` (Dockerfile) to build turboserve with all optional dependencies enabled.

    podman build -t turboserve .

### Run your image
The image expects to find static content at `/wwwroot`, so a volume containing your content can be mounted.

    docker run --rm -p 8080:8080 -v ./www:/wwwroot turboserve

To bring your own `turboserve.conf`, simply mount it at `/turboserve.conf`.

    podman run --rm -p 8080:8080 -v ./turboserve.conf:/turboserve.conf turboserve
...
This example shows how to run turboserve with socket activation and Podman on a Linux host.
...
2. Start a login shell for the user _test_
   ...
   ```
3. Clone the turboserve git repository to _~/turboserve_
4. Build the image
   ```
   podman build -t turboserve ~/turboserve
   ```
5. Create directories
   ...
   ```
6. Create the file _~/turboserve.conf_ with the contents
   ```
...
8. Create the file _~/.config/containers/systemd/my.container_ with the contents
   ...
   [Container]
   Network=none
   Image=localhost/turboserve
   Volume=/home/test/turboserve.conf:/turboserve.conf:Z
   Volume=/home/test/web:/web:Z
   ```
   The option `:Z` is needed on SELinux systems.
   As __turboserve__ only needs to communicate over the socket-activated socket, it's possible to use `Network=none`. See the article [How to limit container privilege with socket activation](https://www.redhat.com/sysadmin/socket-activation-podman).
...
11. Start the socket
    ...
    ```
12. Download the example text file from the turboserve web server
    ```
...
These are some of the quotes found in the wild about turboserve.  They're presented
in no particular order.  Contributions are appreciated:

> "turboserve is like a classic, according to the definition given by Italian
> -- writer Italo Calvino: you can read it again and again" [Antonio
> Piccolboni](https://asceticprogrammer.info/book)

> "I read turboserve's source code. Especially, the part of using coroutine was
...
> "For the server side, we're using turboserve, which can handle 100k+ reqs/s.
...
> "The best performer is turboserve, a newcomer" -- [InfoQ](https://www.infoq.com/news/2015/04/web-frameworks-benchmark-2015/)

> "I've never had a chance to thank you for turboserve.  It inspired me a lot to
> develop [Zewo](https://github.com/Zewo/Zero)" --
> [@paulofariarl](https://twitter.com/paulofariarl/status/707926806373003265)

> "Let me say that turboserve is a thing of beauty.  I got sucked into reading the
...
> "Nice work with turboserve! I haven't looked _that_ carefully yet but so far I
> like what I saw.  You definitely have the right ideas." --
> [@thinkingfish](https://twitter.com/thinkingfish/status/521574267612196864)

> "turboserve is a work of art. Every time I read through it, I am almost always
> awe-struck." --
> [@neurodrone](https://twitter.com/neurodrone/status/359296080283840513)

> "For Round 10, turboserve has taken the crown" --
...
> "turboserve was a complete failure" [dermetfan](http://dermetfan.net/posts/zig-with-c-web-servers.html)
